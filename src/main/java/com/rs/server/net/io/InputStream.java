...
public final class InputStream extends Stream {
	...
	public InputStream(final int capacity) {
	...
	public void checkCapacity(final int length) {
		if (offset + length >= buffer.length) {
			final byte[] newBuffer = new byte[(offset + length) * 2];
	...
	public void addBytes(final byte[] b, final int offset, final int length) {
		checkCapacity(length - offset);
		System.arraycopy(b, offset, buffer, this.offset, length);
		this.length += length - offset;
	}

	public int readPacket(final Player player) {
		final int id = 0xff & readUnsignedByte()
				- player.getIsaacKeyPair().inKey().getNextValue();
		if (id < 128)
			return id;
		return (id - 128 << 8)
				+ (readUnsignedByte() - player.getIsaacKeyPair().inKey()
						.getNextValue());
	}
	...
	public void readBytes(final byte buffer[], final int off, final int len) {
	...
	public int readBigSmart() {
		/*
		 * if(SettingsManager.SETTINGS.CLIENT_BUILD < 670) return readUnsignedShort();
		 */
	...
	public long readLong() {
		...
		final long l1 = readInt() & 0xffffffffL;
		return (l << 32) + l1;
	}
