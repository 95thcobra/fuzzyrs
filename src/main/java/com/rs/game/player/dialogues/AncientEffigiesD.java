	...
	public void run(int interfaceId, int componentId) {
		if (stage == -1) {
			...
			if (player.getSkills().getLevel(
				...
				player.getPackets()
						.sendGameMessage(
								"You're required at least " + AncientEffigies.getRequiredLevel(itemId) + " " + Skills.SKILL_NAME[(Integer) player.getTemporaryAttributtes()
								                  .get("skill1")]+ " to investigate this ancient effigy.");
			} else {
				player.getTemporaryAttributtes().put("skill", skill1);
				sendDialogue(SEND_2_TEXT_INFO, new String[] {
			...
			if (player.getSkills().getLevel(
				...
				player.getPackets()
						.sendGameMessage(
								"You're required at least "+ AncientEffigies.getRequiredLevel(itemId) + " " + Skills.SKILL_NAME[(Integer) player
					       .getTemporaryAttributtes().get("skill1")] + " to investigate this ancient effigy.");
			} else {
