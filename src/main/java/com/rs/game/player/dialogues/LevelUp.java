...
public final class LevelUp extends Dialogue {
	...
	public void start() {
		...
		if (level == 99 || level == 120) {
				World.sendWorldMessage("<col=F20505>News: "+player.getDisplayName()+" has achieved "+level+" "+Skills.SKILL_NAME[skill]+".", false);
		}
        if (player.getSkills().getXp(skill) == 200000000) {
            for (Player p : World.getPlayers()) {
                p.sm("<col=F20505>News: "+ player.getDisplayName() + " has just achieved 200 million experience in "+name+ ".");
            }
        }
	}
