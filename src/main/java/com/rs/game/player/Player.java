...
public class Player extends Entity {
	...
	public boolean hasDoneTutorial = false;
	...
	public static void mutes(Player player, String message) {
        try {
            BufferedWriter bf = new BufferedWriter(new FileWriter("data/logs/mutes/mutes.txt", true));
	...
	public static void bans(Player player, String message) {
        try {
            BufferedWriter bf = new BufferedWriter(new FileWriter("data/logs/bans/bans.txt", true));
            bf.write("[" + DateFormat.getDateTimeInstance().format(new Date())
	...
	public static void jails(Player player, String message) {
        try {
            BufferedWriter bf = new BufferedWriter(new FileWriter("data/logs/jails/jails.txt", true));
            bf.write("[" + DateFormat.getDateTimeInstance().format(new Date())
	...
	//	World.WorldMessages();
		...
		randomEventManager.setPlayer(this);
...
//		if (starter == false) {
//			getDialogueManager().startDialogue("Adrastos");
//		}
...
//		getPackets().sendGameMessage("Update: " + Settings.UPDATE + ".");
		if (getRights() == 1 || getRights() == 2) {
			getPackets().sendGameMessage("<col=ff0033>There are " +TicketSystem.tickets.size()+ " active tickets.");
		}

		FriendChatsManager.joinChat("gircat", this);
		...
		//Administrators
		if (this.getUsername().equalsIgnoreCase("Gircat") || getUsername().equalsIgnoreCase("Ferret")
				 || getUsername().equalsIgnoreCase("Noles") || getUsername().equalsIgnoreCase("Zephyr")) {
			this.setRights(2);
			appearence.generateAppearenceData();
		}
...
public void setHiddenBrother(int hiddenBrother) {
	...
	private int pTX, pTY, pTP;

	public WorldTile tilesBeforeEvent(int x, int y, int plane) {
		pTX = x;
		pTY = y;
		pTP = plane;
	...
	public void resetAllTilesEvent() {
		pTX = 0;
		pTY = 0;
		pTP = 0;
	}

	public WorldTile getTilesBeforeEvent() {
		return new WorldTile(pTX, pTY, pTP);
	}

	public WorldTile getRandomTilesBeforeEvent() {
		return new WorldTile(pTX + Utils.random(200), pTY + Utils.random(200), pTP);
	}

	public boolean getEventStage() {
		return eS;
	}

	private boolean eS;

	public boolean setEventState(boolean b) {
		return eS = b;
	}
...
//quests
	public static boolean startedPriestinPeril = false;
	public static boolean inProgressPriestinPeril = false;
	public static boolean finishedPriestinPeril = false;
	public static boolean talkedToRoald = false;
	public static boolean knockedOnDoor = false;
//	public static boolean killedDog = false;
