...
public class Notes {
	...
	public static class Note {
		...
		private int value;
		public void setColor(int Notes) {
			this.value= value;
		}
	...
	/*	WorldTasksManager.schedule(new WorldTask() {
			@Override
			public void run(){
		if (PriestInPeril.startedPriestinPeril == false){

			notes.add(new Note(new StringBuilder("Priest in Peril"), 255));
		
		} else 
			if (PriestInPeril.startedPriestinPeril == true) {
				notes.add(new Note(new StringBuilder("Priest in Peril"), 255));
			}
	}*/
	//}, 0, 5);
