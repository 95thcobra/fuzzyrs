...
public class FairyRings {
    ...
    public static boolean checkAll(Player player) {
		if (player.getEquipment().getWeaponId() == 772 || player.getEquipment().getWeaponId() == 9084)
		    return true;
		player.getPackets().sendGameMessage("You can't use the Fairy Network if you don't have a Dramen Staff wielded.");
		return false;
    }
    ...
    private static void resetRingHash(Player player) {
		...
		for (int i = 0; i < 3; i++)
		    player.getVarBitManager().sendVarBit(2341 + i, 0);
    }
    ...
    public static void handleDialButtons(final Player player, int componentId) {
		...
		for (int i = 0; i < 3; i++)
		    player.getVarBitManager().sendVarBit(2341 + i, locationArray[i] == 1 ? 3 : locationArray[i] == 3 ? 1 : locationArray[i]);
