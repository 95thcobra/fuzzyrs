...
public class PlayerDesign {
	...
	public static void changeGender(Player player, boolean male) {
		...
		if (male) {
			...
			player.getAppearence().setBodyStyle(0, 5);
			player.getAppearence().setBodyStyle(1, 14);
		} else {
			PlayerDesign[] set = getFemaleSet(1);
			player.getAppearence().setLooks(set[0].looks);
			player.getAppearence().copyColors(set[0].colors);
			player.getAppearence().setBodyStyle(0, 141);
			player.getAppearence().setBodyStyle(1, 57);
		}
		...
		if (screen.intValue() == 3) {
			if (buttonId == 128) {
				if (player.getAppearence().isMale()) {
					...
					player.getAppearence().setBodyStyle(3, arms == -1 ? 1000 : arms);
