...
public class ClanWars {
	 ...
	 */
	public static WorldTile[][] ARENA_TELEPORTS = {
			{ new WorldTile(2883, 5941, 0), new WorldTile(2883, 5900, 0) },
			{ new WorldTile(2815, 5511, 0), new WorldTile(2891, 5515, 0) },
			{ new WorldTile(2993, 5558, 0), new WorldTile(2891, 5515, 0) },
			{ new WorldTile(2930, 5668, 0), new WorldTile(2892, 5641, 0) },
			{ new WorldTile(2883, 5941, 0), new WorldTile(2883, 5900, 0) } };
	 ...
	 * the following order Platue, Turrets, Quarry, Forest, Classic.
	 */
	public static WorldTile[][] DEATH_TELEPORTS = {
			{ new WorldTile(2851, 5931, 0), new WorldTile(2851, 5909, 0) },
			{ new WorldTile(2815, 5511, 0), new WorldTile(2891, 5515, 0) }, // done
			{ new WorldTile(2908, 5686, 0), new WorldTile(2915, 5537, 0) },
			{ new WorldTile(2929, 5667, 0), new WorldTile(2892, 5661, 0) },
			{ new WorldTile(2883, 5941, 0), new WorldTile(2883, 5900, 0) } };
	...
	public static WorldTile CLANWARS_LOBBY = new WorldTile(1, 1, 0);// TODO
	...
	public void addFightTime(long fightTime) {
		this.fightTime = fightTime + Utils.currentTimeMillis();
	}
	...
	public static class ClanChallengeInterface {
		...
		public static void openInterface(Player player) {
			if (player.getCurrentFriendChatOwner() == null) {
				...
				return;
			} else if (player.getCurrentFriendChat().getRank(
					kills, player.getUsername()) <= 2) {
				player.getPackets().sendGameMessage(
						"You need to be a higher rank to start a war.");
				return;
			}
			player.getInterfaceManager().sendInterface(791);
			player.getPackets().sendUnlockIComponentOptionSlots(791, 141, 0,
					63, 0);
		}
		...
		public static void sendClanInterface(Player player) {
			for (Player teamPlayer : player.getClanWars().getTeam()) {
				teamPlayer.getPackets().sendConfigByFile(5280,
