...
public class DuelArena extends Controller {
    ...
    private void openDuelScreen(final Player target, final boolean ifFriendly) {
        if (!ifFriendly) {
            sendOptions(player);
    ...
    private void accept(final boolean firstStage) {
        if (!hasTarget())
            return;
        final boolean accepted = (Boolean) player.getTemporaryAttributtes()
                .get("acceptedDuel");
        final boolean targetAccepted = (Boolean) target
                .getTemporaryAttributtes().get("acceptedDuel");
        final DuelRules rules = player.getLastDuelRules();
    ...
    protected void closeDuelInteraction(final boolean started,
                                        final DuelStage duelStage) {
            ...
            WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                    player.getControllerManager()
                            .startController(DuelController.class);
                }
            });
        ...
        final Controller controller = oldTarget.getControllerManager()
                .getController();
        if (controller == null)
            return;
        final DuelArena targetConfiguration = (DuelArena) controller;
        if (targetConfiguration.hasTarget()) {
            ...
            } else if (duelStage == DuelStage.NO_SPACE) {
    ...
    public void addItem(final int slot, final int amount) {
        ...
        final Item[] itemsBefore = player.getLastDuelRules().getStake()
                .getItemsCopy();
    ...
    public void removeItem(final int slot, final int amount) {
        ...
        final Item[] itemsBefore = player.getLastDuelRules().getStake()
                .getItemsCopy();
        final int maxAmount = player.getLastDuelRules().getStake()
                .getNumberOf(item);
        if (amount < maxAmount) {
            item = new Item(item.getId(), amount);
        } else {
    ...
    private void refreshItems(final Item[] itemsBefore) {
        final int[] changedSlots = new int[itemsBefore.length];
        int count = 0;
        for (int index = 0; index < itemsBefore.length; index++) {
            final Item item = player.getLastDuelRules().getStake().getItems()[index];
            if (item != null)
    ...
    private void refresh(final int... slots) {
    ...
    private void openConfirmationScreen(final boolean ifFriendly) {
        player.getInterfaceManager().sendInterface(ifFriendly ? 639 : 626);
        refreshScreenMessage(false, ifFriendly);
    }

    private void refreshScreenMessages(final boolean firstStage,
                                       final boolean ifFriendly) {
        refreshScreenMessage(firstStage, ifFriendly);
        ((DuelArena) target.getControllerManager().getController())
                .refreshScreenMessage(firstStage, ifFriendly);
    }

    private void refreshScreenMessage(final boolean firstStage,
                                      final boolean ifFriendly) {
    ...
    private String getAcceptMessage(final boolean firstStage) {
    ...
    private void sendOptions(final Player player) {
    ...
    public void endDuel(final Player victor, final Player loser,
                        final boolean removeLoserControler) {
        for (final Item item : player.getLastDuelRules().getStake().getItems()) {
            if (item == null) {
                continue;
            }
            if (loser.isDead() && (!victor.isDead())) {
                victor.getInventory().addItem(item);
            } else {/* DUPE */
                // victor.getPackets().sendGameMessage("Both you and your opponent died at same time and didnt receive stake items.");
                // loser.getPackets().sendGameMessage("Both you and your opponent died at same time and didnt receive stake items.");
            }
        }

        for (final Item item : target.getLastDuelRules().getStake().getItems()) {
            if (item == null) {
                continue;
            }
            victor.getInventory().addItem(item);
        }
        if (loser.getControllerManager().getController() != null
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            @Override
            public void run() {
                victor.getControllerManager().startController(DuelController.class);
                loser.getControllerManager().startController(DuelController.class);
            }
        }, 2);
    }

    private void startEndingTeleport(final Player player) {
        final WorldTile tile = LOBBY_TELEPORTS[Utils
                .random(LOBBY_TELEPORTS.length)];
    ...
    private void beginBattle(final boolean started) {
        if (started) {
            final WorldTile[] teleports = getPossibleWorldTiles();
            final int random = Utils.getRandom(1);
    ...
    public boolean keepCombating(final Entity victim) {
        ...
        if (player.getCombatDefinitions().getSpellId() > 0 && rules.getRule(2)
            ...
            for (final Item item : FUN_WEAPONS) {
                if (rules.getRule(8)
    ...
    private WorldTile[] getPossibleWorldTiles() {
        ...
        final WorldTile[] locations = new WorldTile[2];
        final int[] arenaBoundariesX = {3337, 3367, 3336};
        final int[] arenaBoundariesY = {3246, 3227, 3208};
        final int[] maxOffsetX = {14, 14, 16};
        final int[] maxOffsetY = {10, 10, 10};
        ...
        if (player.getLastDuelRules().getRule(25)) {
            final int direction = Utils.getRandom(1);
            if (direction == 0) {
                finalX--;
            } else {
                finalY++;
                            ...
                            case 47:
                                switch (packetId) {
                                    ...
                                    case WorldPacketsDecoder.ACTION_BUTTON4_PACKET:
                                        final Item item = player.getInventory().getItems()
                                                .get(slotId);
                                        if (item == null)
                                            return false;
