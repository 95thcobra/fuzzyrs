...
public class StealingCreationGame extends Controller {
	...
	public void sendInterfaces() {
		// player.getInterfaceManager().sendTab(809,
		// player.getInterfaceManager().hasRezizableScreen() ? 11 : 27);
		sendUpdateScore(30);
	}

	private void sendUpdateScore(final int value) {
		score += value;
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() == 39546) {
			...
			player.getPackets()
					.sendHideIComponent(813, 98 + (index * 2), false);
			return false;
		}
		switch (object.getId()) {
		case 39553:
		...
		case 39550:
			...
			StealingCreation.startDynamicSkill(
					player,
					object,
					name == null ? null
							: StealingCreation.getAnimationForBase(
		...
		case 39563:
		case 39562:
			calculatedId = 4 - (object.getId() - (object.getId() == 39569 ? 39566
					: 39562));
	...
	private void processKilnExchange(final int componentId, final int packetId) {
		final int itemId = StealingCreation.SACRED_CLAY[index];
		int amount = 0;
		...
		if (StealingCreation
				.checkSkillRequriments(player, StealingCreation
						.getRequestedKilnSkill(componentId - 37), index)) {
			if ((amount != 0 && StealingCreation.proccessKilnItems(player,
					componentId, index, itemId, amount)))
				return;
		}
	}
	...
	private void calculateKilnIndex() {
		for (int index = 4; index >= 0; index--) {
			final int item = StealingCreation.SACRED_CLAY[index];
	...
	public boolean logout() {
		Magic.sendTeleportSpell(player, -1, -1, -1, -1, 0, 0,
				StealingCreation.LOBBY_WORLDTILE, 0, true, -1);
