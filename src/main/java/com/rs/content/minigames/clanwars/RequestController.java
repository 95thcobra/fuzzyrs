...
import com.rs.content.dialogues.impl.ClanWarsViewing;
...
public final class RequestController extends Controller {
	 ...
	 * @param message If the player should be messaged if he was unable to
	 *                challenge.
	 * @return {@code True} if so.
	 */
	private static boolean canRequest(final Player player, final Player target,
									  final boolean message) {
		String text = null;
		if (player.getClanManager().getClan() == null) {
			text = "You need to be in a clan to start a war.";
		} else if (target.getClanManager().getClan() == null) {
			text = "This player is not in a clan.";
		} else if (player.getClanManager().getRank(player) == 0) {
			text = "You need to be a higher rank to start a war.";
		} else if (target.getClanManager().getRank(player) == 0) {
			text = "This player does not have a high enough rank to accept your challenge.";
		} else if (player.getClanManager() == target.getClanManager()) {
			text = "You can't challenge players in the same clan as you are in.";
		} else if (player.getClanManager().getClanWars() != null) {
			text = "Your clan is already in a war.";
		} else if (target.getClanManager().getClanWars() != null) {
	 ...
	 * @return {@code True} if so.
	 */
	public static boolean inWarRequest(final Player player) {
	...
	public boolean processButtonClick(final int interfaceId,
		...
		final ClanWars clanWars = (ClanWars) player.getTemporaryAttributtes()
				.get("clan_wars");
		if (clanWars == null)
			return true;
	...
	public boolean canPlayerOption1(final Player target) {
		...
		player.setCoordsEvent(new CoordsEvent(target, () -> {
            if (!canRequest(player, target, true))
                return;
            if (target == null
                    || target.hasFinished()
                    || !target.withinDistance(player, 14)
					|| !(target.getControllerManager().getController() instanceof RequestController)) {
            ...
            if (target.getTemporaryAttributtes().get("clan_request_p") == player) {
                final ClanWars clanWars = new ClanWars(target.getClanManager(), player.getClanManager());
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		case 28213:
			if (player.getClanManager() == null
					|| player.getClanManager().getClanWars() == null) {
				player.getDialogueManager().startDialogue(ClanWarsViewing.class);
				return false;
			}
		...
		case 38698: // Safe FFA portal.
			...
			player.getControllerManager().startController(FfaZone.class, false);
			return false;
		case 38699: // Risk FFA portal.
			player.setNextWorldTile(new WorldTile(3007, 5511, 0));
			player.getControllerManager().startController(FfaZone.class, true);
			return false;
		}
		return true;
	}
	...
	public void decline() {
		final Player other = (Player) player.getTemporaryAttributtes().get(
				"clan_request_p");
		final ClanWars war = (ClanWars) player.getTemporaryAttributtes().get(
				"clan_wars");
