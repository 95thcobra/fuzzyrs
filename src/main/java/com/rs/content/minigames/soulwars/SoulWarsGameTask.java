...
public class SoulWarsGameTask extends GameTask {
    ...
    private ArrayList<Barricade> redBarricades = new ArrayList<>(5);
    private ArrayList<Barricade> blueBarricades = new ArrayList<>(5);
    private Avatar[] avatars = new Avatar[2];
    private int ticks = 1;

    public SoulWarsGameTask(PlayerType type) {
    ...
    public void run() {
        try {
            if (MINUTES_BEFORE_NEXT_GAME.get() < 4)
                return;
            handleAreas();
            ...
            for (Iterator<Player> it = players.iterator(); it.hasNext(); ) {
                ...
                if (player != null && player.getTemporaryAttributtes().remove("soulwars_kicked") != null)
                    it.remove();
                else if (player != null && player.getControllerManager().getController() instanceof GameController) {
                    player.getControllerManager().sendInterfaces();
                    if (showInterfaces)
                        ((GameController) player.getControllerManager().getController()).decreaseActivity();
                    if (ghostTick) {
                        ...
                        if (respawnIndex != null && respawnIndex == callOutGhosts && respawnTile != null && respawnIndex != 1 && player.getAppearance().getTransformedNpcId() != -1) {
        ...
        for (Player player : players) {
            for (int index = 0; index < playerAmounts.length; index++) {
                if (player != null && player.getX() >= AREAS[index][0].getX() && player.getY() >= AREAS[index][0].getY()
                        && player.getX() <= AREAS[index][1].getX() && player.getY() <= AREAS[index][1].getY() && player.getAppearance().getTransformedNpcId() == -1) {
                    final Teams team = Teams.values()[player.getEquipment().getCapeId() - TEAM_CAPE_INDEX];
                    playerAmounts[index][team.ordinal()]++;
                    if (player.getControllerManager().getController() instanceof GameController)
                        ((GameController) player.getControllerManager().getController()).setAreaIndex(index);
                    break;
                } else if (player != null)
                    if (player.getControllerManager().getController() instanceof GameController)
                        ((GameController) player.getControllerManager().getController()).setAreaIndex(-1);
            }
        }
        for (int index = 0; index < playerAmounts.length; index++) {
    ...
    public void start() {
        reset();
        for (int i = 0; i < 2; i++)
            (avatars[i] = new Avatar(AVATAR_INDEX + i, i == 0 ? new WorldTile(1968, 3251, 0) : new WorldTile(1807, 3210, 0), -1, true)).setRandomWalk(true);
    }
    ...
    public WorldTile getRespawnLocation(Player player, int team) {
        WorldTile respawnLocation = World.getSoulWars().calculateRandomLocation(Teams.values()[team], PlayerType.IN_GAME);
        int respawnIndex = -1;
    ...
    public void increaseKill(int id) {
        kills[id - AVATAR_INDEX]++;
        levels[id - AVATAR_INDEX] = 100;
    }
    ...
    public boolean placeBarricade(Player player, int team) {
        ...
        for (final WorldTile tile : ANTI_CARRICADES) {
