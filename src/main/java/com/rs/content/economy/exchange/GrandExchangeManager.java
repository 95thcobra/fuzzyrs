...
public class GrandExchangeManager implements Serializable {
    ...
    public void openGrandExchange() {
        ...
        player.getPackets().sendUnlockIComponentOptionSlots(105, 206, -1, 0, 0,
                1);
        player.getPackets().sendUnlockIComponentOptionSlots(105, 208, -1, 0, 0,
                1);
    ...
    public void openCollectionBox() {
        ...
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 19, 0, 2, 0, 1);
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 23, 0, 2, 0, 1);
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 27, 0, 2, 0, 1);
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 32, 0, 2, 0, 1);
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 37, 0, 2, 0, 1);
        player.getPackets()
                .sendUnlockIComponentOptionSlots(109, 42, 0, 2, 0, 1);
    }
        ...
        if (interfaceId == 105) {
            switch (componentId) {
                ...
                case 171:
                    ...
                    break;
                case 175:
                    modifyPricePerItem(GrandExchangePriceManager.getPrice(getItemId()));
                    break;
                case 177:
                    editPrice();
                    break;
                case 179:
                    ...
                    break;
                case 181:
                    modifyPricePerItem((int) (Math.ceil(getPricePerItem() * 0.95)));
                    break;
                case 186:
                    confirmOffer();
                    break;
    ...
    public void editAmount() {
        ...
        player.getTemporaryAttributtes().put("GEQUANTITYSET", true);
    ...
    public void editPrice() {
        ...
        player.getTemporaryAttributtes().put("GEPRICESET", true);
        player.getPackets().sendRunScript(
                108,
    ...
    public void sendInfo(Item item) {
        ...
        if (bonuses == null || bonuses.length < 16) {
            return;
        }
        player.getPackets().sendGlobalString(26, "");
        ...
        player.getPackets().sendGlobalString(35, "<br>Attack<br><col=ffff00>+"
    ...
    public void setExtraDetails(int id) {
        setItemId(id);
        setMarketPrice(GrandExchangePriceManager.getPrice(id));
        ...
        if (getType() == 1) {
            if (!GrandExchangeUnlimitedItems.isUnlimited(id)) {
                if (totalbuyamount > 0) {
                    player.getPackets().sendIComponentText(105, 143, "" + ItemExamines.getExamine(new Item(id)) + "<br><br>Best sell price: " + Utils.getFormattedNumber(buyprice, ',') + "<br>Quantity: " + Utils.getFormattedNumber(buyamount, ','));
                    player.getPackets().sendFilteredGameMessage(true, "There are %s %s currently being bought on the Grand Exchange.", Utils.getFormattedNumber(totalbuyamount, ','), ItemDefinitions.getItemDefinitions(id).getName() + "s");
                } else
                    player.getPackets().sendIComponentText(105, 143, "" + ItemExamines.getExamine(new Item(id)) + "<br> <br>There are currently no buy offers for this item.");
            } else {
                player.getPackets().sendIComponentText(105, 143, ItemExamines.getExamine(new Item(id)) + "<br> <br>The Grand Exchange will automatically buy this <br>" + "item.");
            }
        }
        if (getType() == 0) {
            if (GrandExchangeUnlimitedItems.isUnlimited(id)) {
                player.getPackets().sendIComponentText(105, 143, ItemExamines.getExamine(new Item(id)) + "<br> <br>Quantity: Unlimited.");
            } else {
                if (totalsellamount > 0) {
                    player.getPackets().sendIComponentText(105, 143, "" + ItemExamines.getExamine(new Item(id)) + "<br><br>Lowest price: " + Utils.getFormattedNumber(sellprice, ',') + "<br>Quantity: " + Utils.getFormattedNumber(sellamount, ','));
                    player.getPackets().sendFilteredGameMessage(true, "There are %s %s currently being sold on the Grand Exchange.", Utils.getFormattedNumber(totalsellamount, ','), ItemDefinitions.getItemDefinitions(id).getName() + "s");
                } else
                    player.getPackets().sendIComponentText(105, 143, "" + ItemExamines.getExamine(new Item(id)) + "<br> <br>There are currently no sell offers for this item.");
    ...
    public void setItem(Item item, boolean sell) {
        if (item.getId() == ShopCurrency.GOLD.getId() || !ItemConstants.isTradeable(item)) {
            player.getPackets().sendGameMessage(
        ...
        int price = GrandExchangePriceManager.getPrice(item.getId());
    ...
    public void confirmOffer() {
        ...
        if (buy) {
            ...
            int notedId = ItemDefinitions.getItemDefinitions(itemId).getCertId();
