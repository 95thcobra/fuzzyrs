...
public class ItemSets {
    ...
    public static Sets getSetFromItem(int itemId) {
        for (Sets set : Sets.values()) {
            for (int i : set.getItems()) {
                if (i == itemId) {
                    return set;
                }
            }
    ...
    public static void exchangeFromSet(Player player, int slot, int id) {
        ...
        Sets set = getSet(id);
        if (set == null) {
            player.getPackets().sendInventoryMessage(0, slot, "This isn't a set item, you can't break it up into component parts.");
            return;
        }
        if (player.getInventory().getFreeSlots() < set.items.length - 1) {
            player.getPackets().sendInventoryMessage(0, slot, "You don't have enough inventory space for the component parts.");
            return;
        }
        ...
        player.getPackets().sendGameMessage("You successfully traded your item components for a set!");
    }

    public static void exchangeForSet(Player player, int slotId, int itemId) {
        Sets set = getSetFromItem(itemId);
        if (set == null) {
            player.getPackets().sendInventoryMessage(0, slotId, "This item is not part of a set!");
            return;
        }
        for (int i : set.items) {
            if (!player.getInventory().containsItem(i, 1)) {
                player.getPackets().sendInventoryMessage(0, slotId, "You don't have the parts to make up this set.");
                return;
            }
        }
        for (int i : set.items) {
    ...
    public static void sendComponentsBySlot(Player player, int slot, int itemId) {
        ...
        sendComponents(player, slot, itemId);
    }

    public static void examineSet(Player player, int slotId, int id) {
        Sets set = getSet(id);
        if (set == null) {
            player.getPackets().sendInventoryMessage(0, slotId, "This isn't a set item.");
            return;
        }
        player.getPackets().sendInventoryMessage(0, slotId, ItemExamines.getExamine(new Item(id, 1)));
    }

    public static void sendComponents(Player player, int slotId, int id) {
        Sets set = getSet(id);
        if (set == null) {
            player.getPackets().sendInventoryMessage(0, slotId, "This isn't a set item.");
            return;
        ...
        if (message == null)
            return;
        player.getPackets().sendInventoryMessage(0, slotId, message);
    }

    public static void openSets(Player player) {
        player.getInterfaceManager().sendInventoryInterface(644);
        player.getPackets().sendUnlockIComponentOptionSlots(644, 0, 0, 27, 0, 1);
        boolean make = (boolean) player.getTemporaryAttributtes().get("armor_trade");
        if (make) {
            player.getPackets().sendInterSetItemsOptionsScript(644, 0, 93, 4, 7, "Components", "Exchange", "Examine");
        } else {
            player.getPackets().sendInterSetItemsOptionsScript(644, 0, 93, 4, 7, "Exchange", "Examine");
        }
