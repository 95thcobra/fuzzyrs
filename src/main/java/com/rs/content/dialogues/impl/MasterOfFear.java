...
public class MasterOfFear extends Dialogue {
    ...
    public void start() {
        ...
        if (stage == 3) {
            sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE, "Donator cape",
                    "Basket of eggs", "Sled");
        } else {
            sendEntityDialogue(SEND_1_TEXT_CHAT, new String[]{
                            "Master of fear",
                            "Hello, " + player.getDisplayName()
                                    + ", welcome to the Refuge of Fear grotto.",},
                    IS_NPC, 15149, 9843);
        }
        stage = 1;
    }
    ...
    public void run(final int interfaceId, final int componentId) {
        if (stage == 1) {
            sendOptionsDialogue(SEND_DEFAULT_OPTIONS_TITLE, "How do I play?",
                    "Can I see your reward stock?");
            stage = 2;
        } else if (stage == 2) {
            if (componentId == 11) {
