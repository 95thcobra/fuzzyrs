...
public abstract class Dialogue {
    ...
    private static int[] getIComponentsIds(final short interId) {
        int[] childOptions;
        switch (interId) {
    ...
    public static boolean sendNPCDialogueNoContinue(final Player player,
                                                    final int npcId, final int animationId, final String... text) {
        return sendEntityDialogueNoContinue(player, IS_NPC, npcId, animationId,
                text);
    }

    public static boolean sendPlayerDialogueNoContinue(final Player player,
                                                       final int animationId, final String... text) {
     ...
     */
    public static boolean sendEntityDialogueNoContinue(final Player player,
                                                       final int type, final int entityId, final int animationId,
                                                       final String... text) {
        String title = "";
        if (type == IS_PLAYER) {
    ...
    public static boolean sendEntityDialogueNoContinue(final Player player,
                                                       final int type, final String title, final int entityId,
                                                       final int animationId, final String... texts) {
    ...
    public static void closeNoContinueDialogue(final Player player) {
    ...
    public boolean Item(final String... options) {
    ...
    public boolean sendNPCDialogue(final int npcId, final int animationId,
                                   final String... text) {
        return sendEntityDialogue(IS_NPC, npcId, animationId, text);
    }

    public boolean sendPlayerDialogue(final int animationId,
                                      final String... text) {
     ...
     * auto selects title, new dialogues
     */
    public boolean sendEntityDialogue(final int type, final int entityId,
                                      final int animationId, final String... text) {
        String title = "";
        if (type == IS_PLAYER) {
            title = player.getDisplayName();
    ...
    public boolean Options2(final String title, final String... options) {
        player.getInterfaceManager().sendChatBoxInterface(1185);
        player.getPackets().sendIComponentText(1185, 14, title);
        for (int line = 0; line < 2; line++) {
            if (line < options.length) {
                if (line == 0) {
                    player.getPackets().sendIComponentText(1185, 20,
                            options[line]);
                } else if (line == 1) {
                    player.getPackets().sendIComponentText(1185, 25,
                            options[line]);
                }
            }
        }
    ...
    public boolean sendOptionsDialogue(final String title,
     ...
     * new dialogues
     */
    public boolean sendEntityDialogue(final int type, final String title,
                                      final int entityId, final int animationId, final String... texts) {
        final StringBuilder builder = new StringBuilder();
        for (final String text : texts) {
            builder.append(" " + text);
        }
    ...
    public boolean sendDialogue(final String... texts) {
        final StringBuilder builder = new StringBuilder();
    ...
    public boolean sendEntityDialogue(final short interId,
                                      final String[] talkDefinitons, final byte type, final int entityId,
                                      final int animationId) {
        if (type == IS_PLAYER || type == IS_NPC) { // auto convert to new
            // dialogue all old
            // dialogues
            final String[] texts = new String[talkDefinitons.length - 1];
        ...
        final int[] componentOptions = getIComponentsIds(interId);
