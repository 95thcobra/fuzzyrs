package com.rs.content.actions.skills.crafting.spinning;
...
public class Spinning {
    ...
    private static void spin(Player player, SpinningItem item, int itemId) {
        if (player.getSkills().getLevel(Skills.CRAFTING) < item.getSkillRequirement()) {
            player.getPackets().sendGameMessage("You need a Crafting level of " + item.getSkillRequirement() + " to craft this.");
            return;
        }
