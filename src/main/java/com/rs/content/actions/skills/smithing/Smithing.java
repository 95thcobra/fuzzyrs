...
public class Smithing extends Action {
    ...
    public Smithing(final int ticks, final int index) {
        this.index = index;
        this.ticks = ticks;
    }
    ...
    private double getExperience(final Player player) {
        final int levelRequired = bar.getLevel()
                + ForgingInterface.getFixedAmount(bar, bar.getItems()[index]);
        final int barAmount = ForgingInterface.getActualAmount(levelRequired,
                bar, bar.getItems()[index].getId());
    ...
    public enum ForgingBar {
        ...
        static {
            for (final ForgingBar bar : ForgingBar.values()) {
        ...
        ForgingBar(final int barId, final int level,
                   final Item[] items, final double[] experience,
                   final int[] componentChilds) {
            this.barId = barId;
            this.level = level;
            this.items = items;
        ...
        public static ForgingBar forId(final int id) {
            return bars.get(id);
        }

        public static ForgingBar getBar(final Player player) {
            final int smithLevel = player.getSkills().getLevel(Skills.SMITHING);
            for (final ForgingBar bar : bars.values()) {
    ...
    public static class ForgingInterface {
        ...
        public static void handleIComponents(final Player player,
        ...
        private static int getBasedAmount(final Item item) {
            final String def = item.getDefinitions().getName();
        ...
        private static int getFixedAmount(final ForgingBar bar, final Item item) {
        ...
        public static int getForgedAmount(final int id) {
            final String name = ItemDefinitions.getItemDefinitions(id)
                    .getName();
        ...
        public static String[] getStrings(final Player player,
                                          final ForgingBar bar, final int index, final int itemId) {
            if (itemId == -1 || index < 0 || index >= bar.getItems().length)
                return null;
            final StringBuilder barName = new StringBuilder();
            final StringBuilder levelString = new StringBuilder();
            final String name = ItemDefinitions.getItemDefinitions(itemId)
                    .getName().toLowerCase();
            final String barVariableName = bar.toString().toLowerCase();
            int levelRequired = bar.getLevel()
                    + getFixedAmount(bar, bar.getItems()[index]);
            final int barAmount = getActualAmount(levelRequired, bar, itemId);
        ...
        public static int getLevels(final ForgingBar bar, final int slot,
                                    final Player player) {
            final int base = bar.getLevel();
            final int barAmount = getFixedAmount(bar, bar.getItems()[slot]);
        ...
        private static void sendComponentConfigs(final Player player,
                                                 final ForgingBar bar) {
            for (final int i : bar.getComponentChilds()) {
                player.getPackets().sendHideIComponent(SMITHING_INTERFACE,
                        i - 1, false);
            }
        }

        public static int getActualAmount(int levelRequired,
                                          final ForgingBar bar, final int id) {
            if (levelRequired >= 99) {
                levelRequired = 99;
            }
            final int level = levelRequired - bar.getLevel();
            final String name = ItemDefinitions.getItemDefinitions(id)
                    .getName().toLowerCase();
            if (level >= 0 && level <= 4)
                return 1;
        ...
        public static void sendSmithingInterface(final Player player,
                                                 final ForgingBar bar) {
            ...
            for (int i = 0; i < bar.getItems().length; i++) {
                ...
                final String[] name = getStrings(player, bar, i,
