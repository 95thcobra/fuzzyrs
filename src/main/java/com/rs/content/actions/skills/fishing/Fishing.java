package com.rs.content.actions.skills.fishing;
...
public class Fishing extends Action {
    ...
    public Fishing(final FishingSpots spot, final NPC npc) {
        this.spot = spot;
        this.npc = npc;
    ...
    private int getFishingDelay(final Player player) {
        final int playerLevel = player.getSkills().getLevel(Skills.FISHING);
        final int fishLevel = spot.getFish()[fishId].getLevel();
        final int modifier = spot.getFish()[fishId].getLevel();
    ...
    private int getSpecialFamiliarBonus(final int id) {
        switch (id) {
            case 6796:
            case 6795:// rock crab
                return 1;
    ...
    private int getRandomFish(final Player player) {
        int random = Utils.random(spot.getFish().length);
        final int difference = player.getSkills().getLevel(Skills.FISHING)
    ...
    private void addFish(final Player player) {
        final Item fish = new Item(spot.getFish()[fishId].getId(),
                multipleCatch ? 2 : 1);
    ...
    private boolean hasFishingSuit(final Player player) {
    ...
    private String getMessage(final Item fish) {
    ...
    public enum Fish {
        ...
        Fish(final int id, final int level, final double xp) {
            this.id = id;
            this.level = level;
            this.xp = xp;
        }
    ...
    public enum FishingSpots {
        ...
        FishingSpots(final int id, final int option, final int tool,
                     final int bait, final Animation animation, final Fish... fish) {
            this.id = id;
            this.tool = tool;
            this.bait = bait;
        ...
        public static FishingSpots forId(final int id) {
            return spot.get(id);
        }
