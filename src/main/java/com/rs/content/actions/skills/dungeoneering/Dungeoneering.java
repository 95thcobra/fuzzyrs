package com.rs.content.actions.skills.dungeoneering;
...
import com.rs.world.npc.dungeonnering.DungeonBoss;
...
public final class Dungeoneering {
	...
	public static DungeonStructure generateDungeonStructure(int floorType,
		...
		int room11 = Utils.random(10);
		int room22 = Utils.random(10);
		...
		private Dungeon(final int floor, final int complexity, final int size,
			...
			CoresManager.SLOW_EXECUTOR.execute(() -> {
				try {
					...
					if (mapBaseCoords != null) {
						RegionBuilder.cutMap(mapBaseCoords[0], mapBaseCoords[1], ratio[0] * 2, ratio[1] * 2, 0);
					}
