...
public class SummoningScroll {
    ...
    public static void sendInterface(final Player player) {
        ...
        final Object options[] = new Object[]{78, 1,
    ...
    private enum Scrolls {
        ...
        Scrolls(final int scrollId, final double xp, final int pouchId,
                        final int reqLevel) {
         ...
         */
        static {
            for (final Scrolls pouch : Scrolls.values()) {
         ...
         *
         * @param pouchId The pouchId item id.
         * @return The {@code SummoningPouch} {@code Object}, <br>
         * or {@code null} if the pouchId didn't exist.
         */
        public static Scrolls get(final int pouchId) {
            return SCROLLS.get(pouchId);
        }
    }

    public static void createScroll(final Player player, final int itemId,
                                    int amount) {
        final Scrolls scroll = Scrolls.get(itemId);
        if (scroll == null) {
            player.getPackets()
                    .sendGameMessage(
                            "You do not have the pouch required to create this scroll.");
            return;
        }
        if (amount == 28
                || amount > player.getInventory().getItems()
                .getNumberOf(scroll.pouchId)) {
            amount = player.getInventory().getItems()
                    .getNumberOf(scroll.pouchId);
        }
        if (!player.getInventory().containsItem(scroll.pouchId, 1)) {
            player.getPackets().sendGameMessage(
                    "You do not have enough "
                            + ItemDefinitions
                            .getItemDefinitions(scroll.pouchId)
                            .getName().toLowerCase()
                            + "es to create "
                            + amount
                            + " "
                            + ItemDefinitions
                            .getItemDefinitions(scroll.scrollId)
                            .getName().toLowerCase() + "s.");
            return;
        }
        if (player.getSkills().getLevel(Skills.SUMMONING) < scroll.reqLevel) {
            player.getPackets().sendGameMessage(
                    "You need a summoning level of "
                            + scroll.reqLevel
                            + " to create "
                            + amount
                            + " "
        ...
        player.getPackets().sendGameMessage(
                "You have created some "
                        + ItemDefinitions.getItemDefinitions(scroll.scrollId)
                        .getName().toLowerCase() + "s.");
    }
