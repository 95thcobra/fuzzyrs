...
public final class Runecrafting {
	...
	public static boolean isTiara(final int id) {
	...
	public static void enterAirAltar(final Player player) {
		enterAltar(player, new WorldTile(2841, 4829, 0));
	}

	private static void enterAltar(final Player player, final WorldTile dest) {
		player.getPackets().sendGameMessage(
				"A mysterious force grabs hold of you.");
		player.useStairs(-1, dest, 0, 1);
	}

	public static void enterMindAltar(final Player player) {
		enterAltar(player, new WorldTile(2792, 4827, 0));
	}

	public static void enterWaterAltar(final Player player) {
		enterAltar(player, new WorldTile(3482, 4838, 0));
	}

	public static void enterEarthAltar(final Player player) {
		enterAltar(player, new WorldTile(2655, 4830, 0));
	}

	public static void enterFireAltar(final Player player) {
		enterAltar(player, new WorldTile(2574, 4848, 0));
	}

	public static void enterBodyAltar(final Player player) {
		enterAltar(player, new WorldTile(2522, 4825, 0));
	}

	public static void craftEssence(final Player player, final int rune,
			final int level, final double experience,
			final boolean pureEssOnly, final int... multipliers) {
		final int actualLevel = player.getSkills()
				.getLevel(Skills.RUNECRAFTING);
		...
		if (!pureEssOnly) {
			final int normalEss = player.getInventory().getItems()
	...
	public static boolean hasRcingSuit(final Player player) {
	...
	public static void locate(final Player p, final int xPos, final int yPos) {
		String x = "";
		String y = "";
		final int absX = p.getX();
		final int absY = p.getY();
		if (absX >= xPos) {
			x = "west";
		}
		if (absY > yPos) {
	...
	public static void checkPouch(final Player p, final int i) {
		if (i < 0)
			return;
		p.getPackets().sendGameMessage(
	...
	public static void fillPouch(final Player p, final int i) {
		if (i < 0)
			return;
	...
	public static void emptyPouch(final Player p, final int i) {
		if (i < 0)
			return;
