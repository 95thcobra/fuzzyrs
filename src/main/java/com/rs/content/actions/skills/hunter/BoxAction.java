...
public class BoxAction extends Action {
	...
	public BoxAction(final HunterEquipment hunt) {
		this.hunt = hunt;
	}

	public int getTrapAmount(final Player player) {
		int level = 20;
		int trapAmount = 2;
		for (int i = 0; i < 3; i++) {
	...
	private boolean checkAll(final Player player) {
		...
		final int trapAmt = getTrapAmount(player);
		...
		final List<WorldObject> objects = World.getRegion(player.getRegionId())
				.getSpawnedObjects();
		if (objects != null) {
	...
	public enum HunterNPC {
		...
		SPINED_LARUPIA(5104,
				...
				new Animation(5231));// might as well try converting it lol
		...
		static {
			for (final HunterNPC npcs : HunterNPC.values()) {
				npc.put(npcs.npcId, npcs);
			}
			for (final HunterNPC objets : HunterNPC.values()) {
		...
		HunterNPC(final int npcId, final Item[] item, final int level,
				  final double xp, final HunterEquipment hunter,
				  final int successfulTransformObjectId,
				  final int failedTransformObjectId,
				  final Animation successCatchAnim, final Animation failCatchAnim) {
			this.npcId = npcId;
			this.item = item;
			this.level = level;
			this.xp = xp;
		...
		public static HunterNPC forId(final int id) {
			return npc.get(id);
		}

		public static HunterNPC forObjectId(final int id) {
	...
	public enum HunterEquipment {
		...
		HunterEquipment(final int itemId, final int objectId,
						final Animation pickUpAnimation, final int baseLevel) {
			this.itemId = itemId;
