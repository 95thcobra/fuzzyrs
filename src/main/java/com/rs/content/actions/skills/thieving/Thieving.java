...
public class Thieving {
	...
	public static boolean isGuard(final int npcId) {
	...
	public static void checkGuards(final Player player) {
		NPC guard = null;
		int lastDistance = -1;
		for (final int regionId : player.getMapRegionsIds()) {
			...
			for (final int npcIndex : npcIndexes) {
				...
				final int distance = Utils.getDistance(npc.getX(), npc.getY(),
	...
	public static boolean pickDoor(final Player player, final WorldObject object) {
		...
		final int decreasedChance = (Integer) player.getTemporaryAttributtes()
				.get("numbFingers");
		final int level = Utils.getRandom(thievingLevel
				+ (increasedChance - decreasedChance)) + 1;
		final double ratio = level / (Utils.getRandom(45 + 5) + 1);
		...
		PacketHandlerManager.get(DoorHandler.class).process(player, object, 1500 + Utils.getRandom(1000));
		return true;
	}

	private static int getIncreasedChance(final Player player) {
		int chance = 0;
	...
	public enum Stalls {
		...
		Stalls(final int objectId, final int level, final int[] item,
			   final int amount, final double seconds,
			   final double experience, final int replaceObject) {
		...
		public int getItem(final int count) {
			return item[count];
		}
