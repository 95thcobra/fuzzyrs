...
public class Room {
	...
	public String toString() {
		return "[Room][" + rotation + "]" + ", "
				+ Arrays.toString(room.getDoorDirections()) + ", "
				+ (getChunkX() << 3) + ", " + (getChunkY() << 3);
	}
	...
	public boolean containsOnlySouthDoor() {
		return room.getDoorDirections().length == 1
				&& room.getDoorDirections()[0] == DungeonConstants.SOUTH_DOOR;
	}
