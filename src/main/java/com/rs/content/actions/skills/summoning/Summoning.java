package com.rs.content.actions.skills.summoning;
...
public class Summoning implements Serializable {
    ...
    public static Familiar createFamiliar(final Player player,
                                          final Pouches pouch) {
        final String loc = "com.rs.world.npc.familiar.";
        try {
            final Familiar fam = (Familiar) Class.forName(loc + NPCDefinitions.getNPCDefinitions(pouch.getNpcId()).name.replace(" ", "")
                    .replace("-", "")).getConstructor(new Class[]{Player.class, Pouches.class, WorldTile.class, Integer.TYPE, Boolean.TYPE})
    ...
    public static void spawnFamiliar(final Player player, final Pouches pouch) {
        ...
        if (!player.getControllerManager().canSummonFamiliar())
            return;
        final ItemDefinitions def = ItemDefinitions.getItemDefinitions(pouch
                .getPouchId());
        if (def == null)
            return;
        final HashMap<Integer, Integer> skillReq = def
                .getWearingSkillRequiriments();
        boolean hasRequiriments = true;
        if (skillReq != null) {
            for (final int skillId : skillReq.keySet()) {
                if (skillId > 24 || skillId < 0) {
                    continue;
                }
                final int level = skillReq.get(skillId);
        ...
        if (player.getCurrentFriendChat() != null) {
            final ClanWars war = player.getClanManager().getClanWars();
            if (war != null) {
                if (war.get(ClanWars.Rules.NO_FAMILIARS)
    ...
    public static int amountCanCreate(final Player player, final int itemId, final int amount) {
        final SummoningPouch pouch = SummoningPouch.get(itemId);
        return player.getInventory().getItems().getNumberOf(pouch.getItems()[2]);
    }

    public static boolean hasPouch(final Player player) {
    ...
    public static void createPouch(final Player player, final int itemId,
                                   int amount) {
        final SummoningPouch pouch = SummoningPouch.get(itemId);
        if (pouch == null) {
            player.getPackets().sendGameMessage(
        ...
        final int amountToCreate = amountCanCreate(player, itemId, amount);
        ...
        int i;
        for (i = 0; i < amount; i++) {
            for (final Item item : pouch.getItems()) {
        ...
        if (i == 1) {
            ...
            player.getPackets().sendGameMessage(
                    "You infuse some "
                            ...
                            .getName().toLowerCase() + "es.");
        } else
            return;
        player.setNextAnimation(new Animation(9068));
    }

    public static void attackDreadnipTarget(final NPC target,
                                            final Player player) {
        if (target.isDead()) {
            ...
            return;
                ...
                && target == player.getFamiliar()) {
            player.getPackets().sendGameMessage("");
            return;
        }
        closeDreadnipInterface(player);
        final DreadNip npc = new DreadNip(player, 14416, player, -1, false);
        if (target == npc)
            return;
    ...
    public static void openDreadnipInterface(final Player player) {
    ...
    public static void closeDreadnipInterface(final Player player) {
        player.getInterfaceManager().closeInventory();
    ...
    public void setPouches(final Pouches pouches) {
    ...
    public static final class Pouches implements Serializable {
        ...
        static {
            ...
            pouches = new HashMap<>();
            Pouches apouches[] = ENUM$VALUES;
            final int j = (apouches = values()).length;
            for (int i = 0; i < j; i++) {
                final Pouches pouch = apouches[i];
        ...
        private Pouches(final String s, final int i, final int npcId,
                        final int configId, final int pouchId, final int level,
                        final double useExp, final double creationExp, final long time,
                        final int spawnCost, final int scrollId) {
            super();
            this.npcId = npcId;
        ...
        public static Pouches forId(final int id) {
         ...
         * @param pouches
         */
        public static Pouches valueOf(final String s) {
