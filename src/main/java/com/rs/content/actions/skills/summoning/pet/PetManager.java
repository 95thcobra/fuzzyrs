package com.rs.content.actions.skills.summoning.pet;
...
public final class PetManager implements Serializable {
	 ...
	 */
	public boolean spawnPet(final int itemId, final boolean deleteItem) {
		final Pets pets = Pets.forId(itemId);
		if (pets == null)
			return false;
		...
		if (!hasRequirements(pets))
			return true;
		final int baseItemId = pets.getBabyItemId();
		...
		final int id = pets.getItemId(details.getStage());
		if (itemId != id) {
			player.getPackets().sendGameMessage(
					"This is not the right pet, grow the pet correctly.");
			return true;
		}
		final int npcId = pets.getNpcId(details.getStage());
		if (npcId > 0) {
			final Pet pet = new Pet(npcId, itemId, player, player, details);
			this.npcId = npcId;
			this.itemId = itemId;
	 ...
	 */
	private boolean hasRequirements(final Pets pet) {
		switch (pet) {
		case TZREK_JAD:
			/*
			 * if (!player.isExtremeDonator()) {
			 * player.getPackets().sendGameMessage
			 * ("You need to be Extreme donator to use this pet."); return
			 * false; } if (!player.isCompletedFightCaves()) {
			 * player.getPackets().sendGameMessage(
			 * "You need to complete at least one fight cave minigame to use this pet."
			 * ); return false; } if (!player.isWonFightPits()) {
			 * player.getPackets().sendGameMessage(
			 * "You need to win at least one fight pits minigame to use this pet."
			 * ); return false; }
			 */
			return true;
		case SARADOMIN_OWL:
		...
		case CHAMELEON:
			/*
			 * if (!player.isDonator()) { player.getPackets().sendGameMessage(
			 * "You need to be donator to use this pet."); return false; }
			 */
			return true;
		case BABY_DRAGON_1:
		case BABY_DRAGON_2:
		...
		case GIANT_WOLPERTINGER:
			return player.getRank().isMinimumRank(PlayerRank.MOD);
		}
		return true;
	}
	 ...
	 *            The pet NPC.
	 */
	public void eat(final int foodId, final Pet npc) {
		...
		for (final int food : pets.getFood()) {
			if (food == foodId) {
	 ...
	 */
	public void removeDetails(final int itemId) {
		final Pets pets = Pets.forId(itemId);
		if (pets == null)
			return;
	 ...
	 */
	public void setTrollBabyName(final String trollBabyName) {
