...
public enum Pets {
	 ...
	 * TzRek-Jad pet.
	 */
	TZREK_JAD(21512, -1, -1, 3604, -1, -1, 0.0, 99), EASTER_BUNNY(1961, -1, -1,
			7410, -1, -1, 0.0, 99), GNOME_NOOB(3257, -1, -1, 2139, -1, -1, 0.0,
			99), PIRATE_PETE(7505, -1, -1, 2825, -1, -1, 0.0, 99), FREEZY(
			24512, -1, -1, 15634, -1, -1, 0.0, 99), ROCK(21250, -1, -1, 2626,
			-1, -1, 0.0, 99), RED_EGG(5076, -1, -1, 9186, -1, -1, 0.0, 99), GREEN_EGG(
			5077, -1, -1, 9187, -1, -1, 0.0, 99), BLUE_EGG(5078, -1, -1, 7816,
			-1, -1, 0.0, 99),
	/**
	 ...
	 */
	static {
		for (final Pets pet : Pets.values()) {
	 ...
	 */
	public static Pets forId(final int itemId) {
	 ...
	 */
	public static boolean hasPet(final Player player) {
		for (final int itemId : babyPets.keySet()) {
			if (player.getInventory().containsOneItem(itemId))
				return true;
		}
		for (final int itemId : grownPets.keySet()) {
			if (player.getInventory().containsOneItem(itemId))
				return true;
		}
		for (final int itemId : overgrownPets.keySet()) {
			if (player.getInventory().containsOneItem(itemId))
				return true;
		}
		return false;
	 ...
	 */
	Pets(final int babyItemId, final int grownItemId,
		 final int overgrownItemId, final int babyNpcId,
		 final int grownNpcId, final int overgrownNpcId,
		 final double growthRate, final int summoningLevel,
		 final int... food) {
	 ...
	 * @return The NPc id.
	 */
	public int getNpcId(final int stage) {
		switch (stage) {
		case 0:
			return babyNpcId;
		case 1:
	 ...
	 * @return The item id.
	 */
	public int getItemId(final int stage) {
		switch (stage) {
		case 0:
			return babyItemId;
		case 1:
