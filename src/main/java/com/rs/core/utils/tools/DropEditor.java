...
public class DropEditor extends javax.swing.JFrame {
    ...
    private final DefaultMutableTreeNode dropsNode = new DefaultMutableTreeNode(
            "Drops");
    private final DefaultTreeModel dropsTreeModel = new DefaultTreeModel(
            dropsNode);
    private final NPCDropsFileManager loader = new NPCDropsFileManager();
    ...
    public DropEditor() {
        ...
        final SortedMap<Integer, ArrayList<Drop>> values = new TreeMap<Integer, ArrayList<Drop>>(
        ...
        for (final Entry<Integer, ArrayList<Drop>> s : loader.getDropArray()
                .entrySet()) {
    ...
    private void initComponents() {
        ...
        dropTable.addMouseListener(new MouseListener() {
            ...
            public void mouseClicked(final MouseEvent arg0) {
                if (arg0.getButton() == MouseEvent.BUTTON3) {
                    final int row = dropTable.rowAtPoint(arg0.getPoint());
        ...
        final javax.swing.GroupLayout dropsPanelLayout = new javax.swing.GroupLayout(
                dropsPanel);
        ...
        removeDrop.addActionListener(new ActionListener() {
        ...
        dumpDropMenuItem.addActionListener(new ActionListener() {
    ...
    protected void testDropRate(final ActionEvent arg0) {
        final ArrayList<Drop> drops = loader.getDropArray().get(
                Integer.parseInt(dropsTree.getLastSelectedPathComponent()
                        .toString()));
        final Drop drop = drops.get(dropTable.getSelectedRow());
        final String testAmount = JOptionPane
    ...
    protected void dumpSpecificDropMenuItemActionPerformed(final ActionEvent evt) {
        try {
    ...
    protected void dumpDropMenuItemActionPerformed(final ActionEvent evt) {
        final Thread dumpThread = new Thread() {
    ...
    public void dump(final int id) throws Exception {
        ...
        if (def != null && def.name != null && !def.name.equals("")
            ...
            // InputStream instr;
            final URL url = new URL("http://runescape.wikia.com/wiki/"
                    + def.name.replace(" ", "_").replace(".", ""));
            final URLConnection curl = url.openConnection();
            curl.setDoInput(true);
            if (pageExists(curl)) {
                final InputStream instr = curl.getInputStream();
                String s;
                final ArrayList<Drop> list = new ArrayList<Drop>();
                ...
                if (instr != null) {
                                            ...
                                            maxAmount = Integer
                                                    .parseInt(s
                                                            ...
                                                            s.substring(s
                                                                    .indexOf("&ndash;") + 7),
                                                            ""));
                                        } catch (final NumberFormatException d) {
                                        }
                                    }
                                } else if (s.contains(",")) {
                    ...
                    if (list.size() > 0) {
                        final Drop[] finallist = (Drop[]) list.toArray();
    ...
    private boolean pageExists(final URLConnection curl) {
        try {
    ...
    private boolean mapHasNpc(final NPCDefinitions def) {
        try {
            for (final Entry<Integer, ArrayList<Drop>> e : loader
                    .getDropArray().entrySet()) {
    ...
    private double getRate(final ItemDefinitions def, int i) {
        if (def.getGEPrice() > 10000000) {
            i = 5;
        }
        switch (i) {
            case 1:
    ...
    private double getPricedPercent(final ItemDefinitions def) {
        if (def.getGEPrice() > 0 && def.getGEPrice() < 1000)
            return 75 + (int) (Math.random() * ((99 - 75) + 1));
        else if (def.getGEPrice() > 1000 && def.getGEPrice() < 100000)
            return 55 + (int) (Math.random() * ((74 - 55) + 1));
        else if (def.getGEPrice() > 100000 && def.getGEPrice() < 1000000)
            return 25 + (int) (Math.random() * ((54 - 25) + 1));
        else if (def.getGEPrice() > 100000 && def.getGEPrice() < 1000000)
            return random.nextInt(10);
        else
    ...
    public DefaultMutableTreeNode searchNode(final String nodeStr) {
        DefaultMutableTreeNode nodeToSearch = null;
        final Enumeration<?> e = dropsNode.breadthFirstEnumeration();
    ...
    private void searchFieldActionPerformed(final ActionEvent evt) {// GEN-FIRST:event_searchFieldActionPerformed
        try {
            final DefaultMutableTreeNode n = searchNode(searchField.getText());
            final TreePath path = new TreePath(dropsTreeModel.getPathToRoot(n));
    ...
    private void dropsTreeValueChanged(
            final javax.swing.event.TreeSelectionEvent evt) {// GEN-FIRST:event_dropsTreeValueChanged
    ...
    private void deleteSelectedActionPerformed(
            final ActionEvent evt) {// GEN-FIRST:event_deleteSelectedActionPerformed
        try {
            final int npcID = Integer.parseInt(dropsTree
                    .getLastSelectedPathComponent().toString());
            final ArrayList<Drop> drops = loader.getDropArray().get(npcID);
            drops.remove(dropTable.getSelectedRow());
            final Drop[] d = new Drop[drops.size()];
            drops.toArray(d);
    ...
    private void addNewActionPerformed(final ActionEvent evt) {// GEN-FIRST:event_addNewActionPerformed
        // try {
        final int npcid = Integer.parseInt(dropsTree
                .getLastSelectedPathComponent().toString());
    ...
    private void addNewButtonActionPerformed(
            final ActionEvent evt) {// GEN-FIRST:event_addNewButtonActionPerformed
    ...
    private void removeDropActionPerformed(final ActionEvent evt) {// GEN-FIRST:event_removeDropActionPerformed
        try {
    ...
    private void repackDropActionPerformed(final ActionEvent evt) {// GEN-FIRST:event_repackDropActionPerformed
        packFile();
                                            ...
                                            Double.parseDouble(dropTable
                                                    .getValueAt(
                                                            dropTable
                                                                    .getSelectedRow(),
                                                            1).toString()));
                            final String amount = (String) dropTable
                                    .getValueAt(dropTable.getSelectedRow(), 2);
                            if (amount.contains("-")) {
                                loader.getDropArray()
                                        .get(npcId)
                                        .get(dropTable.getSelectedRow())
