...
public class IComponentDefinitions {
	...
	public static IComponentDefinitions getInterfaceComponent(final int id,
			final int component) {
		final IComponentDefinitions[] inter = getInterface(id);
		if (inter == null || component >= inter.length)
			return null;
		return inter[component];
	}

	public static IComponentDefinitions[] getInterface(final int id) {
		...
		if (icomponentsdefs[id] == null) {
			...
			for (int i = 0; i < icomponentsdefs[id].length; i++) {
				final byte[] data = Cache.STORE.getIndexes()[3].getFile(id, i);
				if (data != null) {
					final IComponentDefinitions defs = icomponentsdefs[id][i] = new IComponentDefinitions();
					defs.ihash = i + (id << 16);
					if (data[0] != -1)
	...
	static int method925(final int i) {
		return (i & 0x3fda8) >> 11;
	}

	@SuppressWarnings("unchecked")
	final void decode(final InputStream stream) {
		...
		if (type == 5) {
			...
			final int i_18_ = stream.readUnsignedByte();
		...
		if (type == 6) {
			...
			final boolean bool = (0x1 & i_19_) == 1;
		...
		final int i_20_ = stream.read24BitInt();
		...
		if ((i_21_ ^ 0xffffffff) != -1) {
			...
			for (/**/; i_21_ != 0; i_21_ = stream.readUnsignedByte()) {
				final int i_22_ = (i_21_ >> 4) - 1;
				i_21_ = stream.readUnsignedByte() | i_21_ << 8;
				i_21_ &= 0xfff;
				if (i_21_ == 4095) {
					i_21_ = -1;
				}
				final byte b_23_ = (byte) stream.readByte();
				if ((b_23_ ^ 0xffffffff) != -1) {
					aBoolean4802 = true;
				}
				final byte b_24_ = (byte) stream.readByte();
		...
		final int i_25_ = stream.readUnsignedByte();
		final int i_26_ = 0xf & i_25_;
		if (i_26_ > 0) {
		...
		final int i_28_ = i_25_ >> 4;
		if ((i_28_ ^ 0xffffffff) < -1) {
			final int i_29_ = stream.readUnsignedByte();
		...
		if ((i_28_ ^ 0xffffffff) < -2) {
			final int i_31_ = stream.readUnsignedByte();
		...
		if (newInt >= 0) {
			final int i_33_ = stream.readUnsignedByte();
			for (int i_34_ = 0; i_33_ > i_34_; i_34_++) {
				final int i_35_ = stream.read24BitInt();
				final int i_36_ = stream.readInt();
				aHashTable4823.put((long) i_35_, i_36_);
			}
			final int i_37_ = stream.readUnsignedByte();
			for (int i_38_ = 0; i_38_ < i_37_; i_38_++) {
				final int i_39_ = stream.read24BitInt();
				final String string = stream.readJagString();
	...
	private int[] method4150(final InputStream buffer) {
		final int i = buffer.readUnsignedByte();
		if (i == 0)
			return null;
