...
public final class NPCDefinitions {
	...
	public static NPCDefinitions getNPCDefinitions(final int id) {
		NPCDefinitions def = npcDefinitions.get(id);
		if (def == null) {
			def = new NPCDefinitions(id);
			def.method694();
			final byte[] data = Cache.STORE.getIndexes()[18].getFile(
		...
		if (opcode != 1) {
			if (opcode == 2) {
				...
				if (opcode >= 30 && (opcode ^ 0xffffffff) > -36) {
					...
					if (opcode == 41) {
						final int i = stream.readUnsignedByte();
						...
						final int i_24_ = (short) stream.readUnsignedShort();
						int i_25_ = 0;
						...
						final int i_29_ = (short) stream.readUnsignedShort();
						int i_30_ = 0;
						...
						if (opcode == 93) {
							...
							if ((opcode ^ 0xffffffff) == -99) {
								...
								if (opcode == 106 || opcode == 118) {
									...
									final int i_56_ = stream.readUnsignedByte();
									transformTo = new int[2 + i_56_];
									for (int i_57_ = 0; i_56_ >= i_57_; i_57_++) {
										transformTo[i_57_] = stream
												.readUnsignedShort();
											...
											if (opcode == 114) {
												...
												if (opcode != 121) {
																			...
																			if ((opcode ^ 0xffffffff) != -140) {
																				if (opcode == 140) {
																					...
																					if (opcode == 143) {
																						...
																						final int aByte821 = stream
																								.readByte();
																						final int aByte824 = stream
																								.readByte();
																						final int aByte843 = stream
																								.readByte();
																						final int aByte855 = stream
																								.readByte();
																					} else if (opcode == 158) {
																						final byte aByte833 = (byte) 1;
																					} else if (opcode == 159) {
																						final byte aByte833 = (byte) 0;
																					} else if (opcode == 162) { // added
																						// opcode
																						aBoolean3190 = true;
																					} else if (opcode == 163) { // added
																						// opcode
																						final int anInt864 = stream
																								.readUnsignedByte();
																					} else if (opcode == 164) {
																						final int anInt848 = stream
																								.readUnsignedShort();
																						final int anInt837 = stream
																								.readUnsignedShort();
																					} else if (opcode == 165) {
																						final int anInt847 = stream
																								.readUnsignedByte();
																					} else if (opcode == 168) {
																						final int anInt828 = stream
																								.readUnsignedByte();
																					} else if (opcode >= 170
																							&& opcode < 176) {
																						...
																						for (int i_60_ = 0; i > i_60_; i_60_++) {
																							final boolean stringInstance = stream
																									.readUnsignedByte() == 1;
																							final int key = stream
																									.read24BitInt();
																							Object value;
																							if (stringInstance) {
																								value = stream
													...
													for (int i_62_ = 0; ((i_62_ ^ 0xffffffff) > (i ^ 0xffffffff)); i_62_++) {
														final int i_63_ = (stream
																.readUnsignedByte());
														final int[] is = (anIntArrayArray840[i_63_] = (new int[3]));
														is[0] = (stream
																.readByte());
														is[1] = (stream
																.readByte());
