...
public class RenderAnimDefinitions {
	...
	public static RenderAnimDefinitions getRenderAnimDefinitions(
			final int emoteId) {
		...
		if (emoteId == -1)
			return null;
		final byte[] data = Cache.STORE.getIndexes()[2].getFile(32, emoteId);
	...
	private void readValues(final InputStream stream, final int opcode) {
		...
		if (opcode == 54) {
			...
			final int anInt1260 = stream.readUnsignedByte() << 6;
			@SuppressWarnings("unused")
			final int anInt1227 = stream.readUnsignedByte() << 6;
		} else if (opcode == 55) {
			final int[] anIntArray1246 = new int[12];
			final int i_14_ = stream.readUnsignedByte();
			anIntArray1246[i_14_] = stream.readUnsignedShort();
		} else if (opcode == 56) {
			final int[][] anIntArrayArray1217 = new int[12][];
			final int i_12_ = stream.readUnsignedByte();
