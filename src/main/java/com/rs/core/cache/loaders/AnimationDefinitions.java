...
public class AnimationDefinitions {
	...
	public static AnimationDefinitions getAnimationDefinitions(
			final int emoteId) {
		try {
			AnimationDefinitions defs = animDefs.get(emoteId);
			if (defs != null)
				return defs;
			final byte[] data = Cache.STORE.getIndexes()[20].getFile(
					emoteId >>> 7, emoteId & 0x7f);
	...
	public int getEmoteTime() {
		if (anIntArray2153 == null)
			return 0;
		int ms = 0;
		for (final int i : anIntArray2153) {
			ms += i;
		}
		return ms * 30;
	}
	...
	private void readValues(final InputStream stream, final int opcode) {
		if ((opcode ^ 0xffffffff) == -2) {
			final int i = stream.readUnsignedShort();
