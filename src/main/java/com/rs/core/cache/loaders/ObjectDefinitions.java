...
public class ObjectDefinitions {
	...
	private void readValues(final InputStream stream, final int opcode) {
		...
		if (opcode != 1 && opcode != 5) {
																																			...
																																			if (opcode != 166) {
																																				if (opcode == 167) {
																																					...
																																					if (opcode == 169) {
																																						...
																																						final int anInt3383 = stream
																																								.readUnsignedSmart();
																																						// added
																																						// opcode
																																					} else if (opcode == 171) {
																																						final int anInt3362 = stream
																																								.readUnsignedSmart();
																																						// added
																																						// opcode
																																					} else if (opcode == 173) {
																																						final int anInt3302 = stream
																																								.readUnsignedShort();
																																						final int anInt3336 = stream
																																								.readUnsignedShort();
																																						// added
																																						// opcode
																																					} else if (opcode == 177) {
			...
			final boolean aBoolean1162 = false;
			if (opcode == 5 && aBoolean1162) {
				skipReadModelIds(stream);
			}
			final int i_73_ = stream.readUnsignedByte();
			...
			for (int i_74_ = 0; i_74_ < i_73_; i_74_++) {
				...
				final int i_75_ = stream.readUnsignedByte();
	...
	private void skipReadModelIds(final InputStream stream) {
		final int length = stream.readUnsignedByte();
		for (int index = 0; index < length; index++) {
			stream.skip(1);
			final int length2 = stream.readUnsignedByte();
	...
	private static int getArchiveId(final int i_0_) {
		return i_0_ >>> -1135990488;
	}

	public static ObjectDefinitions getObjectDefinitions(final int id) {
		ObjectDefinitions def = objectDefinitions.get(id);
		if (def == null) {
			def = new ObjectDefinitions();
			def.id = id;
			final byte[] data = Cache.STORE.getIndexes()[16].getFile(
