...
public class GraphicDefinitions {
	...
	public static GraphicDefinitions getAnimationDefinitions(
			final int emoteId) {
		GraphicDefinitions defs = animDefs.get(emoteId);
		if (defs != null)
			return defs;
		final byte[] data = Cache.STORE.getIndexes()[21].getFile(
	...
	public static void main(final String... s) {
		...
		for (int i = 0; i < 5000; i++) {
			final GraphicDefinitions def = GraphicDefinitions
	...
	public void readValues(final InputStream stream, final int opcode) {
		if (opcode != 1) {
							...
							if (opcode == 11) { // added opcode
								...
								if ((opcode ^ 0xffffffff) == -42) {
									...
									for (int i_0_ = 0; i > i_0_; i_0_++) {
										aShortArray1455[i_0_] = (short) (stream
												.readUnsignedShort());
										aShortArray1435[i_0_] = (short) (stream
												.readUnsignedShort());
									}
								}
							} else {
