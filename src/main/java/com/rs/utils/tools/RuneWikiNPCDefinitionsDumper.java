...
public class RuneWikiNPCDefinitionsDumper {
    ...
    public static boolean dumpNPC(final int npcId) {
        ...
        try {
            ...
            for (int linesCount = 0; linesCount < page.getLines().size(); linesCount++) {
                final String line = page.getLines().get(linesCount);
                if (line.equalsIgnoreCase(COMBAT_LEVEL_LINE)) {
                    linesCount++;
                    final String combatLevelsLine = getFormatedString(page
                            .getLines().get(linesCount));
                    final String[] levels = combatLevelsLine.split(",");
                    linesCount += 3;
                    final String hpCheckLine = page.getLines().get(linesCount);
                    if (!hpCheckLine.equalsIgnoreCase(HP_LINE))
                        throw new RuntimeException("invalid hp line.");
                    linesCount++;
                    final String lifePointsLine = getFormatedString(page
                            .getLines().get(linesCount));
                    final String[] lifePoints = lifePointsLine.split(",");
                    for (int i = 0; i < levels.length; i++) {
                        final int level = Integer.valueOf(levels[i]);
                        if (level == defs.combatLevel) {
                            hitpoints = Integer
                                    .valueOf(lifePoints[i >= lifePoints.length ? lifePoints.length - 1
                                            : i]);
                            break;
                        }
                    }
                    ...
                    linesCount++;
                    final String isAggresive = getFormatedString(page
                            .getLines().get(linesCount));
                    ...
                    linesCount++;
                    final String attackStyleLine = page.getLines()
                            .get(linesCount).toLowerCase();
                    ...
                    linesCount++;
                    final String attackSpeedLine = page.getLines().get(
                            linesCount);
                    if (attackSpeedLine.contains("Unknown")) {
                        attackSpeed = 4;
                    } else {
                        final String speed = attackSpeedLine.substring(
                                attackSpeedLine.indexOf("Speed") + 5,
                                attackSpeedLine.indexOf(".gif"));
                        attackSpeed = Integer.valueOf(speed) + 1;
                    }
                }
            }
            if (maxhit == -1 || hitpoints == -1 || attackStyle == -1
                    || attackSpeed == -1)
                return false;
            System.out.println("id: " + npcId + ", " + defs.name);
            ...
            try {
                final BufferedWriter writer = new BufferedWriter(
                        new FileWriter("testsrer.txt", true));
                final String t = attackStyle == 0 ? "MELEE"
                        : attackStyle == 1 ? "RANGE" : "MAGE";
                final String a = aggressive ? "AGRESSIVE" : "PASSIVE";
                writer.write("//" + defs.name + " - " + defs.combatLevel);
                writer.newLine();
                writer.write(npcId + " - " + hitpoints + " -1 -1 -1 "
                        + attackSpeed + " 1 60 " + maxhit + " " + t + " -1 -1 "
                        + a);
                writer.newLine();
                writer.flush();
                writer.close();
    ...
    public static String getFormatedString(final String s) {
        return s.replace("</th><td>", "")
                .replace("to", ",")
                .replace("-", ",")
                .replace("/", ",")
                .replace("Varies", "0")
                ...
                .replace("<br", "")
                .replace(
                        "<br/>(0in<ahref=\"/wiki/Daemonheim\"title=\"Daemonheim\">Daemonheim</a>)",
                        "")
                .replace(
                        "(13in<i><ahref=\"/wiki/The_Restless_Ghost\"title=\"TheRestlessGhost\">TheRestlessGhost</a></i>)",
                        "13,").replace("(variesinDaemonheim:...", "");
