...
import com.rs.world.npc.fightkiln.HarAkenTentacle;
...
public class PlayerCombat extends Action {
	...
	public static int getKnifeThrowGfxId(final int weaponId) {
		// knives TODO ALL
		if (weaponId == 868)
			return 225;
	...
	public static int getSpecialAmmount(final int weaponId) {
		switch (weaponId) {
			...
			case 22211:
			case 22213:
			case 24186: // custom spec weap. no idea for what boss yet.
				return 100;
	...
	public static boolean fullVanguardEquipped(final Player player) {
	...
	public static boolean usingGoliathGloves(final Player player) {
		final String name = player.getEquipment()
				.getItem(Equipment.SLOT_SHIELD) != null ? player.getEquipment()
				.getItem(Equipment.SLOT_SHIELD).getDefinitions().getName()
				.toLowerCase() : "";
	...
	public static boolean fullVeracsEquipped(final Player player) {
		final int helmId = player.getEquipment().getHatId();
		final int chestId = player.getEquipment().getChestId();
		final int legsId = player.getEquipment().getLegsId();
		final int weaponId = player.getEquipment().getWeaponId();
	...
	public static boolean fullDharokEquipped(final Player player) {
		final int helmId = player.getEquipment().getHatId();
		final int chestId = player.getEquipment().getChestId();
		final int legsId = player.getEquipment().getLegsId();
		final int weaponId = player.getEquipment().getWeaponId();
	...
	public static boolean fullVoidEquipped(final Player player,
										   final int... helmid) {
		...
		final int legsId = player.getEquipment().getLegsId();
		final boolean hasLegs = legsId != -1
		...
		final int torsoId = player.getEquipment().getChestId();
		final boolean hasTorso = torsoId != -1
	...
	public static int getWeaponAttackEmote(final int weaponId,
										   final int attackStyle) {
		if (weaponId != -1) {
			if (weaponId == -2) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if (weaponName.contains("chinchompa")
						|| weaponName.contains("blisterwood stake"))
					return 2779;
				...
				if (weaponName.contains("staff")
						|| weaponName.contains("obliteration")
				...
				if (weaponName.contains("tzhaar-ket-om")
						|| weaponName.contains("raider axe")) {
					switch (attackStyle) {
						default:
							return 13691;
					}
				...
				if (weaponName.contains("virtus wand")) {
					switch (attackStyle) {
						case 1:
							return 16348;
						case 2:
				...
				if (weaponName.contains("2h sword")
						...
						|| weaponName.equals("sword of edicts")
				...
				if (weaponName.contains("rapier")
						...
						|| weaponName.contains("frostmourne")) {
					switch (attackStyle) {
						case 2:
							return 13048;
						default:
	 ...
	 */
	public static int isRanging(final Player player) {
		final int weaponId = player.getEquipment().getWeaponId();
		if (weaponId == -1)
			return 0;
		final String name = ItemDefinitions.getItemDefinitions(weaponId)
				.getName();
		if (name != null) { // those dont need arrows
			if (name.contains("knife") || name.contains("dart")
					...
					|| name.equalsIgnoreCase("decimation")
					|| name.equalsIgnoreCase("boogie bow")
					|| name.equalsIgnoreCase("blisterwood stake")
					|| name.equalsIgnoreCase("virtus wand")
					|| name.equalsIgnoreCase("blisterwood staff")
	 ...
	 */
	private static boolean hasPolyporeStaff(final Player player) {
		final int weaponId = player.getEquipment().getWeaponId();
	...
	private boolean forceCheckClipAsRange(final Entity target) {
		return target instanceof NexMinion || target instanceof HarAken
	...
	public int processWithDelay(final Player player) {
		...
		if (spellId > 0) {
			final boolean manualCast = spellId != 65535 && spellId >= 256;
			final Item gloves = player.getEquipment().getItem(
			...
			final int delay = mageAttack(player, manualCast ? spellId - 256
					: spellId, !manualCast);
	...
	public Entity[] getMultiAttackTargets(final Player player,
			final int maxDistance, final int maxAmtTargets) {
		...
		if (target.isAtMultiArea()) {
			y: for (final int regionId : target.getMapRegionsIds()) {
				...
				if (target instanceof Player) {
					final List<Integer> playerIndexes = region
							.getPlayerIndexes();
	...
	private double getSpecialAccuracyModifier(final Player player) {
		final Item weapon = player.getEquipment()
				.getItem(Equipment.SLOT_WEAPON);
		if (weapon == null)
			return 1;
		final String name = weapon.getDefinitions().getName().toLowerCase();
		...
		if (name.contains("anchor") || name.contains("magic longbow"))
			return 2;
		if (name.contains("dragon claws") || name.contains("obliteration")
				|| name.contains("decimation") || name.contains("annihilation")
				|| name.contains("keenblade"))
			return 2;
	...
	public boolean hasZammySpear(final Player player) {
		final int weaponId = player.getEquipment().getWeaponId();
		return weaponId == 11716;
	}

	public int mageAttack(final Player player, final int spellId,
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 36:// bind
				...
				mage_hit_gfx = 181;
				base_mage_xp = 60.5;
				final Hit magicHit = getMagicHit(player,
			...
			case 55:// snare
				...
				mage_hit_gfx = 180;
				base_mage_xp = 91.1;
				final Hit snareHit = getMagicHit(player,
			...
			case 81:// entangle
				...
				mage_hit_gfx = 179;
				base_mage_xp = 91.1;
				final Hit entangleHit = getMagicHit(player,
			...
			case 86: // teleblock
				if (target instanceof Player
					...
					base_mage_xp = 80;
					block_tele = true;
					final Hit hit = getMagicHit(player,
							getRandomMagicMaxHit(player, 30));
					delayMagicHit(2, hit);
			...
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				mage_hit_gfx = 1019;
				base_mage_xp = 70;
				final int boost = (player.getSkills().getLevelForXp(
						Skills.MAGIC) - 77) * 10;
			...
			case 38: // Miasmic burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						mage_hit_gfx = 1849;
						base_mage_xp = 42;
						final int damage = getRandomMagicMaxHit(player, 240);
						delayMagicHit(2, getMagicHit(player, damage));
			...
			case 39: // Miasmic barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						mage_hit_gfx = 1854;
						base_mage_xp = 54;
						final int damage = getRandomMagicMaxHit(player, 320);
						delayMagicHit(2, getMagicHit(player, damage));
						if (target.getTemporaryAttributtes().get(
								"miasmic_immunity") != Boolean.TRUE) {
							if (target instanceof Player) {
			...
			case 30:// Smoke burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 36;
						max_poison_hit = 20;
						final int damage = getRandomMagicMaxHit(player, 190);
						delayMagicHit(2, getMagicHit(player, damage));
			...
			case 34:// Shadow burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 37;
						reduceAttack = true;
						final int damage = getRandomMagicMaxHit(player, 200);
						delayMagicHit(2, getMagicHit(player, damage));
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 26:// Blood burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 39;
						blood_spell = true;
						final int damage = getRandomMagicMaxHit(player, 210);
						delayMagicHit(2, getMagicHit(player, damage));
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 22:// Ice burst
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						freeze_time = 15000;
						magic_sound = 169;
						final int damage = getRandomMagicMaxHit(player, 220);
			...
			case 31:// Smoke barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 48;
						max_poison_hit = 40;
						final int damage = getRandomMagicMaxHit(player, 270);
						delayMagicHit(2, getMagicHit(player, damage));
			...
			case 35:// shadow barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 49;
						reduceAttack = true;
						final int damage = getRandomMagicMaxHit(player, 280);
						delayMagicHit(2, getMagicHit(player, damage));
						if (!nextTarget) {
							if (damage == -1)
								return false;
			...
			case 27:// blood barrage
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						blood_spell = true;
						max_poison_hit = 40;
						final int damage = getRandomMagicMaxHit(player, 290);
						delayMagicHit(2, getMagicHit(player, damage));
						World.sendProjectile(player, target, 390, 18, 18, 50,
								50, 0, 0);
						if (!nextTarget) {
			...
			case 23: // ice barrage
				...
				attackTarget(getMultiAttackTargets(player), new MultiAttack() {
					...
					public boolean attack() {
						...
						base_mage_xp = 52;
						final int damage = getRandomMagicMaxHit(player, 300);
						final Hit hit = getMagicHit(player, damage);
						delayMagicHit(Utils.getDistance(player, target) > 3 ? 4
								: 2, hit);
						World.sendProjectile(player, target, 368, 60, 32, 50,
								50, 0, 0);
						if (!nextTarget) {
							if (damage == -1)
								return false;
	...
	public int getRandomMagicMaxHit(final Player player, final int baseDamage) {
		final int current = getMagicMaxHit(player, baseDamage);
		if (current <= 0) // Splash.
			return -1;
		...
		if (hit > 0) {
			if (target instanceof NPC) {
				...
				if (n.getId() == 8133 && hasZammySpear(player)) {
					hit += 60;
				}
			}
		}
	 ...
	 * (fullVoidEquipped(player, new int[] { 11663, 11674 })) EA *= 1.5; EA *=
	 ...
	 * (p2.getFamiliar().getMessageIcon() == 6870 || p2.getFamiliar().getMessageIcon() == 6871))
	...
	private int getMagicMaxHit(final Player player, final int baseDamage) {
	...
	private double getMagicAccuracy(final Player e, final int baseDamage,
			final int extraDamage) {
		final int magicLevel = e.getSkills().getLevel(Skills.MAGIC) + 1;
		...
		final double prayer = 1.0 * e.getPrayer().getMageMultiplier();
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			if (weaponId != -1) {
				...
				if (weaponName.contains("throwing axe")
						...
						|| weaponName.contains("toktz-xil-ul")
					...
					final int hit = getRandomMaxHit(player, weaponId,
							attackStyle, true);
					...
					dropAmmo(player, -1);
					/*
					 * if(weaponId == 25202 && target instanceof NPC) {
					 * World.sendProjectile(player, target, 3429, 41, 36, 41,
					 * 32, 15, 0); target.setNextGraphics(new Graphics(3428, 0,
					 * 100)); NPC t = (NPC) target; t.applyHit(new Hit((NPC)
					 * target, target.getHitpoints(), HitLook.RANGE_DAMAGE));
					 * if(t.getMessageIcon() == 13447) { t.sendDeath(player); t.drop(); }
					 * if(t.getMessageIcon() == 8133) { t.sendDeath(player); t.drop(); }
					 * player.getEquipment().deleteItem(25202, 1);
					 * player.getAppearance().resetAppearence();
					 * player.getPackets().sendGameMessage(
					 * "The target falls to it's death as your dart dissapears..."
					 * ); }
					 */
				} else if (weaponName.contains("crossbow")
						|| weaponName.contains("dominion crossbow")) {
				...
				} else if (weaponName.contains("red chinchompa")) {
					attackTarget(getMultiAttackTargets(player),
							new MultiAttack() {
								...
								public boolean attack() {
									...
									World.sendProjectile(player, target, 909,
											41, 16, 31, 16, 0, 0);
				...
				} else if (weaponName.contains("blisterwood stake")) {
					attackTarget(getMultiAttackTargets(player),
							new MultiAttack() {
				...
				} else if (weaponName.contains("boogie bow")) {
					player.setNextAnimation(new Animation(getWeaponAttackEmote(
							weaponId, attackStyle)));
					World.sendProjectile(player, target, 3195, 41, 36, 41, 35,
							0, 0);
					final int hit = getRandomMaxHit(player, weaponId,
							attackStyle, true);
					delayHit(2, weaponId, attackStyle, getRangeHit(player, hit));
					...
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId,
							hit, 249, 41, 36, 41, 35, 0, 0);
				} else if (weaponName.contains("decimation")) {
					player.setNextAnimation(new Animation(getWeaponAttackEmote(
							weaponId, attackStyle)));
					player.setNextGraphics(new Graphics(250));
					World.sendProjectile(player, target, 249, 41, 36, 41, 35,
					...
					checkSwiftGlovesEffect(player, 2, attackStyle, weaponId,
							hit, 249, 41, 36, 41, 35, 0, 0);
				} else if (weaponName.contains("blisterwood staff")) {
					...
					delayMagicHit(
							1,
							getMagicHit(player,
									getRandomMagicMaxHit(player, 490)));
					World.sendProjectile(player, target, 1937, 18, 18, 50, 50,
							0, 0);
				} else if (weaponName.contains("virtus wand")) {
					player.setNextAnimation(new Animation(16348));
					base_mage_xp = 40;
					mage_hit_gfx = -1;
					delayMagicHit(
							1,
					...
					World.sendProjectile(player, target, 1900, 18, 18, 50, 50,
							0, 0);
					...
					if (target instanceof Player) {
						final Player p = (Player) target;
						final Item weapon = p.getEquipment().getItem(3);
						boolean slashBased = weapon != null;
						if (weapon != null) {
							final int slash = p.getCombatDefinitions()
									.getBonuses()[CombatDefinitions.SLASH_ATTACK];
							for (int i = 0; i < 5; i++) {
	 ...
	 */
	private void checkSwiftGlovesEffect(final Player player,
			final int hitDelay, final int attackStyle, final int weaponId,
			final int hit, final int gfxId, final int startHeight,
	...
	public void dropAmmo(final Player player, final int quantity) {
		if (quantity == -2) {
			final int ammoId = player.getEquipment().getAmmoId();
			player.getEquipment().removeAmmo(ammoId, 1);
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utils.getRandom(10) < 2)
					...
					if (capeId != -1
							...
							|| ItemDefinitions.getItemDefinitions(capeId)
									.getName().contains("Completionist cape"))
	...
	public void dropAmmo(final Player player) {
		dropAmmo(player, 1);
	}

	public int getArrowThrowGfxId(final int weaponId, final int arrowId) {
		if (arrowId == 884)
			return 18;
		else if (arrowId == 886)
			return 20;
	...
	public int getArrowProjectileGfxId(final int weaponId, final int arrowId) {
		if (arrowId == 884)
			return 11;
		else if (arrowId == 886)
			return 12;
			...
			return 1120;
		else if (weaponId == 10034)
			return 909;
		else if (weaponId == 21581)
			return -1;
		else if (weaponId == 24474)
			return 3195;
	...
	@SuppressWarnings("unused")
	private int getRangeHitDelay(final Player player) {
	...
	private int meleeAttack(final Player player) {
		...
		final int attackStyle = player.getCombatDefinitions().getAttackStyle();
		final int combatDelay = getMeleeCombatDelay(player, weaponId);
		int soundId = getSoundId(weaponId, attackStyle);
		if (weaponId == -1) {
			final Item gloves = player.getEquipment().getItem(
					Equipment.SLOT_HANDS);
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 21375:
				player.setNextAnimation(new Animation(11971));
				target.setNextGraphics(new Graphics(2108, 0, 100));
				target.getPoison().makePoisoned(130);
				if (target instanceof Player) {
					final Player p2 = (Player) target;
					p2.setRunEnergy(p2.getRunEnergy() > 25 ? p2.getRunEnergy() - 25
							: 0);
			...
			case 23681:
				...
				final int sgsdamage = getRandomMaxHit(player, weaponId,
			...
			case 23680:
				...
				final int damage = getRandomMaxHit(player, weaponId,
						attackStyle, false, true, 1.2, true);
				delayNormalHit(weaponId, attackStyle,
			...
			case 11061: // ancient mace
				...
				player.setNextForceTalk(new ForceTalk(
						"Zaros! Show me your power and destroy the fly infront of me!"));
				player.setNextGraphics(new Graphics(3002));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 20821: // Thok's Sword
				...
				player.setNextForceTalk(new ForceTalk("Skull Crusher!"));
				target.setNextAnimation(new Animation(846));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 13904:
				...
				final int statdamage = getRandomMaxHit(player, weaponId,
						attackStyle, false, true, 1.25, true);
				delayNormalHit(weaponId, attackStyle,
						getMeleeHit(player, statdamage));
				/*
				 * delayNormalHit( weaponId, attackStyle, getMeleeHit( player,
				 * getRandomMaxHit(player, weaponId, attackStyle, false, true,
				 * 1.25, true)));
				 */
				if (target instanceof Player) {
					final Player targetPlayer = ((Player) target);
					int amountLeft;
					if ((amountLeft = targetPlayer.getSkills().drainLevel(
							Skills.DEFENCE, statdamage / 8)) > 0) {
					}
				}
				break;
			case 24455:
				player.setNextAnimation(new Animation(862)); // made something
																// custom of
																// this :)
				player.setNextForceTalk(new ForceTalk(
						"General.... send in the Nuclear Bomb!"));
				target.setNextGraphics(new Graphics(3263));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMeleeHit(
			...
			case 24457:
				...
				target.setNextGraphics(new Graphics(3189));
				delayNormalHit(
						weaponId,
						attackStyle,
						getMagicHit(
			...
			case 18786:
				...
				break;
			case 11700:
				final int zgsdamage = getRandomMaxHit(player, weaponId,
			...
			case 10887: // anchor
				...
				delayNormalHit(
						...
						getMeleeHit(
								player,
								getRandomMaxHit(player, weaponId, attackStyle,
										false, false, 1.22, true)));
				break;

			case 24189: // moia's hands
				player.setNextAnimation(new Animation(7307));
				player.setNextGraphics(new Graphics(448));
	...
	public void playSound(final int soundId, final Player player,
			final Entity target) {
		if (soundId == -1)
			return;
	...
	public int getRandomMaxHit(final Player player, final int weaponId,
			final int attackStyle, final boolean ranging) {
	...
	public int getRandomMaxHit(final Player player, final int weaponId,
			final int attackStyle, final boolean ranging,
			final boolean defenceAffects, final double specMultiplier,
			final boolean usingSpec) {
		...
		if (defenceAffects) {
			...
			if (usingSpec) {
				final double multiplier = /* 0.25 + */getSpecialAccuracyModifier(player);
				att *= multiplier;
			}
			if (def < 0) {
				def = -def;
		...
		if (player.getAuraManager().usingEquilibrium()) {
			final int perc25MaxHit = (int) (max_hit * 0.25);
			hit -= perc25MaxHit;
	...
	private int getMaxHit(final Player player, final int weaponId,
						  final int attackStyle, final boolean ranging,
						  final boolean usingSpec, final double specMultiplier) {
		if (!ranging) {

			/*
			 * //whip hiting 450 no pot no pray? lmao nty int strLvl =
			 * player.getSkills().getLevel(Skills.ATTACK); int strBonus =
			 * player
			 ...
			 * attackStyle); double style = xpStyle == Skills.ATTACK ? 3 :
			 ...
			 * (fullVoidEquipped(player, 11665, 11676)) { hitMultiplier *= 1.4;
			...
			final double strengthLvl = player.getSkills().getLevel(
					Skills.STRENGTH);
			final int xpStyle = CombatDefinitions.getXpStyle(weaponId,
					attackStyle);
			final double styleBonus = xpStyle == Skills.STRENGTH ? 3
			...
			final double baseDamage = 5 + effectiveStrength
					* (1 + (strengthBonus / 64));
			...
			final double rangedLvl = player.getSkills().getLevel(Skills.RANGE);
			final double styleBonus = attackStyle == 0 ? 3
					: attackStyle == 1 ? 0 : 1;
			...
			final double strengthBonus = player.getCombatDefinitions()
					.getBonuses()[CombatDefinitions.RANGED_STR_BONUS];
			final double baseDamage = 5 + (((effectiveStrenght + 8) * (strengthBonus + 64)) / 64);
			return (int) Math.floor(baseDamage * specMultiplier);
		}
	}

	public void delayNormalHit(final int weaponId, final int attackStyle,
			final Hit... hits) {
	...
	private void delayHit(final int delay, final int weaponId,
			final int attackStyle, final Hit... hits) {
			...
			if (hit.getLook() == HitLook.RANGE_DAMAGE
				...
				if (combatXp > 0) {
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE) {
						...
						final int xpStyle = CombatDefinitions.getXpStyle(
								weaponId, attackStyle);
				...
				if (combatXp > 0) {
					...
					if (player.getCombatDefinitions().isDefensiveCasting()
						...
						final int defenceXp = (int) (damage / 7.5);
						if (defenceXp > 0) {
							combatXp -= defenceXp;
							player.getSkills().addXp(Skills.DEFENCE,
									defenceXp / 7.5);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				for (final Hit hit : hits) {
					...
					doDefenceEmote();
					final int damage = hit.getDamage() > target.getHitpoints() ? target
							.getHitpoints() : hit.getDamage();
					...
					if (hit.getLook() == HitLook.RANGE_DAMAGE
						...
						if (combatXp > 0) {
							if (hit.getLook() == HitLook.RANGE_DAMAGE) {
								if (weaponId != -1) {
									final String name = ItemDefinitions
	...
	private int getSoundId(final int weaponId, final int attackStyle) {
		if (weaponId != -1) {
			final String weaponName = ItemDefinitions
					.getItemDefinitions(weaponId).getName().toLowerCase();
	...
	private int getMeleeCombatDelay(final Player player, final int weaponId) {
		if (weaponId != -1) {
			final String weaponName = ItemDefinitions
					.getItemDefinitions(weaponId).getName().toLowerCase();
			...
			if (weaponName.contains("godsword")
					...
					|| weaponName.contains("moia's hands")
	...
	private boolean checkAll(final Player player) {
			...
			if (n instanceof Familiar) {
				...
				if (n.getId() == 14578) {
					if (player.getEquipment().getWeaponId() != 2402
						...
						final int slayerLevel = Combat.getSlayerLevelForNPC(n
								.getId());
		...
		if (distanceX < size && distanceX > -1 && distanceY < size
			...
			if (!player.addWalkSteps(target.getX() + size, target.getY())) {
				...
				if (!player.addWalkSteps(target.getX() - 1, target.getY())) {
					player.resetWalkSteps();
					if (!player.addWalkSteps(target.getX(), target.getY()
							+ size)) {
						player.resetWalkSteps();
		...
		if (player.getCombatDefinitions().isInstantAttack()) {
			...
			if (player.getCombatDefinitions().isUsingSpecialAttack()) {
				...
				final int weaponId = player.getEquipment().getWeaponId();
				final int attackStyle = player.getCombatDefinitions()
						.getAttackStyle();
				switch (weaponId) {
				case 4153:
