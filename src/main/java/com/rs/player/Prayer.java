...
public class Prayer implements Serializable {
	...
	public double getMageMultiplier() {
		...
		if (usingPrayer(0, 4)) {
			...
			final double d = (leechBonuses[2]);
			value += d / 100;
		} else if (usingPrayer(1, 12)) {
			final double d = (5 + leechBonuses[5]);
			value += d / 100;
		}
		return value;
	}
	...
	public double getRangeMultiplier() {
		...
		if (usingPrayer(0, 3)) {
			...
			final double d = (leechBonuses[1]);
			value += d / 100;
		} else if (usingPrayer(1, 11)) {
			final double d = (5 + leechBonuses[4]);
			value += d / 100;
		}
		return value;
	}
	...
	public double getAttackMultiplier() {
		...
		if (usingPrayer(0, 2)) {
			...
			final double d = (leechBonuses[0]);
			value += d / 100;
		} else if (usingPrayer(1, 10)) {
			final double d = (5 + leechBonuses[3]);
			value += d / 100;
		} else if (usingPrayer(1, 19)) {
			final double d = (15 + leechBonuses[8]);
			value += d / 100;
		}
		return value;
	}
	...
	public double getStrengthMultiplier() {
		...
		if (usingPrayer(0, 1)) {
			...
			final double d = (5 + leechBonuses[7]);
			value += d / 100;
		} else if (usingPrayer(1, 19)) {
			final double d = (23 + leechBonuses[10]);
			value += d / 100;
		}
		return value;
	}
	...
	public double getDefenceMultiplier() {
		...
		if (usingPrayer(0, 0)) {
			...
			final double d = (6 + leechBonuses[6]);
			value += d / 100;
		} else if (usingPrayer(1, 19)) {
			final double d = (15 + leechBonuses[9]);
			value += d / 100;
		}
		return value;
	}

	public boolean reachedMax(final int bonus) {
	...
	public void increaseLeechBonus(final int bonus) {
		leechBonuses[bonus]++;
		if (bonus == 0) {
	...
	public void increaseTurmoilBonus(final Player p2) {
	...
	public void adjustStat(final int stat, final int percentage) {
		player.getPackets().sendConfigByFile(6857 + stat, 30 + percentage);
	}

	public void closePrayers(final int prayerId) {
		if (ancientcurses) {
			if (prayerId == 1) {
				if (leechBonuses[0] > 0) {
					player.getPackets()
	...
	public void switchQuickPrayers() {
		...
		if (hasPrayersOn()) {
			...
			boolean hasOn = false;
			int index = 0;
			for (final boolean prayer : quickPrayers[getPrayerBook()]) {
				if (prayer) {
					if (usePrayer(index)) {
						hasOn = true;
					}
	...
	private void closePrayers(final int[]... prayers) {
		for (final int[] prayer : prayers) {
			for (final int prayerId : prayer)
				if (usingQuickPrayer) {
	...
	public void switchPrayer(final int prayerId) {
		if (!usingQuickPrayer)
			if (!checkPrayer())
				return;
		usePrayer(prayerId);
	...
	private boolean usePrayer(final int prayerId) {
	...
	public void processPrayerDrain() {
		if (!hasPrayersOn())
			return;
		final int prayerBook = getPrayerBook();
		final long currentTime = Utils.currentTimeMillis();
		int drain = 0;
		final int prayerPoints = player.getCombatDefinitions().getBonuses()[CombatDefinitions.PRAYER_BONUS];
		for (int index = 0; index < onPrayers[prayerBook].length; index++) {
			if (onPrayers[prayerBook][index]) {
				final long drainTimer = nextDrain[index];
				if (drainTimer != 0 && drainTimer <= currentTime) {
					final int rate = (int) ((prayerDrainRate[getPrayerBook()][index] * 1000) + (prayerPoints * 50));
	...
	public void resetDrainPrayer(final int index) {
	...
	public void setPrayerBook(final boolean ancientcurses) {
		closeAllPrayers();
	...
	private void recalculatePrayer() {
		int value = 0;
		int index = 0;
		for (final boolean prayer : (!usingQuickPrayer ? onPrayers[getPrayerBook()]
	...
	public boolean usingPrayer(final int book, final int prayerId) {
	...
	public void setBoostedLeech(final boolean boostedLeech) {
	...
	public void setPrayerpoints(final int prayerpoints) {
	...
	public void drainPrayer(final int amount) {
	...
	public void restorePrayer(final int amount) {
