...
public class ImpossibleJad extends Controller {
	...
	public static void enterFightCaves(final Player player) {
		player.getControllerManager()
				.startController(ImpossibleJad.class, 1); // start at wave 1
	}
		...
		CoresManager.SLOW_EXECUTOR.execute(new Runnable() {
			...
			public void run() {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (!login) {
							final WorldTile walkTo = getWorldTile(37, 29);
	...
	public boolean processMagicTeleport(WorldTile toTile) {
		player.sendMessage("You are unable to teleport at this time...");
		return false;
	}
