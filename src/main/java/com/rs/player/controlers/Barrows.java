...
public final class Barrows extends Controller {
	...
	public static boolean digIntoGrave(final Player player) {
		for (final Hills hill : Hills.values()) {
			if (player.getPlane() == hill.outBound.getPlane()
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.getControllerManager().startController(Barrows.class);
					}
				});
				return true;
			}
	...
	private void exit(final WorldTile outside) {
		player.setNextWorldTile(outside);
		leave(false);
	}

	private void leave(final boolean logout) {
		if (target != null) {
	...
	public int getRandomBrother() {
		final List<Integer> bros = new ArrayList<Integer>();
	...
	public void sendReward() {
		double percentage = 0;
		for (final boolean died : player.getKilledBarrowBrothers()) {
			if (died) {
				percentage += 12;
			}
		}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if (object.getId() >= 6702 && object.getId() <= 6707) {
			final WorldTile out = Hills.values()[object.getId() - 6702].outBound;
			...
			player.lock(6);
			if (player.getHiddenBrother() != -1) {
				final int brother = getRandomBrother();
				if (brother != -1) {
			...
			final int sarcoId = getSarcophagusId(object.getId());
			if (sarcoId != -1) {
				if (sarcoId == player.getHiddenBrother()) {
					player.getDialogueManager().startDialogue(BarrowsD.class);
				} else if (target != null
	...
	public int getSarcophagusId(final int objectId) {
		switch (objectId) {
		case 66017:
			return 0;
		case 63177:
	...
	public void setBrotherSlained(final int index) {
	...
	public void sendTarget(final int id, final WorldTile tile) {
		if (target != null) {
			target.disapear();
		}
	...
	public void process() {
		...
		if (headComponentId == 0) {
			...
			final int headIndex = getAndIncreaseHeadIndex();
			if (headIndex == -1) {
				resetHeadTimer();
				return;
			}
			...
			player.getPackets().sendIComponentAnimation(9810, 24,
					headComponentId);
			final int activeLevel = player.getPrayer().getPrayerpoints();
			if (activeLevel > 0) {
				final int level = player.getSkills().getLevelForXp(
						Skills.PRAYER) * 10;
	...
	public void sendBrotherSlain(final int index, final boolean slain) {
		player.getPackets().sendConfigByFile(457 + index, slain ? 1 : 0);
	}

	public void sendCreaturesSlainCount(final int count) {
	...
	private enum Hills {
		...
		Hills(final WorldTile outBound, final WorldTile in) {
