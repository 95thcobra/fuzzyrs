...
public class TrollInvasion extends Controller {
    ...
    private final WorldTile OUTSIDE_LOCATION = new WorldTile(2993, 9679, 0);
    /**
     ...
     * @param amt The uses.
     */
    public void setUses(final int amt) {
        this.uses = amt;
    }
     ...
     * @param type the type.
     */
    public void setEohric(final int type) {
        final NPC eohric = World.getNPC(13700);
     ...
     * @param type the type.
     */
    public void switchObject(final int type) {
        if (type == 1) {
            World.spawnObject(
    ...
    /**
     * gets the worldtask tile inside the map.
     *
     * @param mapX
     * @param mapY
     * @return
    ...
    /**
     * Method used to return the wave's we are using for the gametask.
     *
    ...
    public void startWave() {
        ...
        player.getPackets().sendConfig(2215, getWave());
        if (stage != Stages.PLAYING)
            return;
        for (final int id : getComplex()[wave - 1]) {
            if (id == 12921) {
    ...
    /**
     * Method used to destroy the map and end the gametask.
     *
     * @param type the type.
     */
    public void leaveGame(final int type) {
        stage = Stages.DESTROYING;
        final WorldTile outside = new WorldTile(OUTSIDE_LOCATION);
        if (type == 0 || type == 2) {
    ...
    public void commenceGame() {
        ...
        setWave(1);
        setUses(5);
        boundChuncks = RegionBuilder.findEmptyChunkBound(20, 20);
        RegionBuilder.copyAllPlanesMap(353, 443, boundChuncks[0],
                boundChuncks[1], 60);
        final long time = FadingScreen.fade(player);
        FadingScreen.unfade(player, time, new Runnable() {
            ...
            public void run() {
                ...
                player.getInterfaceManager().sendOverlay(447, false);
    ...
    /**
     * Loads data for the start of the gametask.
     */
    ...
    public void processDynamite() {
        final NPC dynamite = World.getNPC(DYNAMITE);
        if (dynamite == null)
            return;
        if (tableDestroyed == false) {
            ...
            final WorldObject table = new WorldObject(9270, 10, 0,
                    new WorldTile(getWorldTile(42, 29)));
    ...
    public void processTrollRunts() {
        final NPC runt = World.getNPC(TROLL_RUNTS);
        final NPC father = World.getNPC(TROLL_FATHER);
        if (runt == null)
            return;
        if (father == null)
            return;
        if (runt.getHitpoints() == 0)
            return;
        final int hpPercent = (runt.getMaxHitpoints() / runt.getHitpoints());
    ...
    public void sendArcherAttack() {
        final int wave = getWave();
        final int complex = getComplexLength();

        if (stage != Stages.PLAYING)
            return;

        final NPC archer = World.getNPC(13701);
        final WorldObject object = new WorldObject(9242, 10, 0, getWorldTile(
                44, 24));
        ...
        if (complexity) {
            for (final int enemy : ADVANCED_WAVES[wave - 1]) {
                if (World.getNPC(enemy) == null) {
                    continue;
                }
                final NPC closeEnemies = World.getNPC(enemy);
                if (closeEnemies == null || closeEnemies.isDead()) {
                    continue;
                }
                final int hpPercent = (closeEnemies.getMaxHitpoints() / closeEnemies
                        .getHitpoints());
            ...
            for (final int enemy : STANDARD_WAVES[wave - 1]) {
                if (World.getNPC(enemy) == null) {
                    continue;
                }
                final NPC closeEnemies = World.getNPC(enemy);
    ...
    public void sendRockAttack() {
        final WorldObject object = new WorldObject(9270, 10, 0, getWorldTile(
                21, 35));
    ...
    /**
     * Method used to finish the gametask and reward the player.
     */
    public void win() {
        if (stage != Stages.PLAYING)
            return;
        ...
        removeControler();
        player.getPackets().sendGameMessage(
                "A spin ticket has been added to your bank!");
        leaveGame(4);
    }

    public void giveReward(final int skill) {
        final int l = player.getSkills().getLevel(skill);
        final int xp = (int) (8 * (wave / (complexity ? 7 : 20)) * (Math.pow(l,
                2) - (2 * l) + 100));
    }
