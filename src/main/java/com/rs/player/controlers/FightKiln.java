...
import com.rs.content.dialogues.impl.FightKilnDialogue;
...
public class FightKiln extends Controller {
    ...
    public static void enterFightKiln(final Player player,
                                      final boolean quickEnter) {
        ...
        if (familiar != null
            ...
            return;
        if (!quickEnter) {
            player.getDialogueManager().startDialogue(FightKilnDialogue.class);
        } else {
            player.getControllerManager()
                    .startController(FightKiln.class, 1); // start at wave 1
        }
    }

    public static void useCrystal(final Player player, final int id) {
                    ...
                    public void run() {
                        ...
                        if (currentWave == 0) { // SCENE 0
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                public void run() {
                                    if (count == 0) {
                                        final WorldTile lookTo = getWorldTile(
                                                29, 39);
                                        player.getPackets().sendCameraLook(
                                                ...
                                                Cutscene.getY(player,
                                                        lookTo.getY()), 3500);
                                        final WorldTile posTile = getWorldTile(
                                                27, 30);
                                        ...
                                        run = player.getRun();
                                        player.setRun(false);
                                        final WorldTile walkTo = getWorldTile(
                                                31, 39);
                                        ...
                                        player.getDialogueManager()
                                                .startDialogue(TokHaarHok.class, 0,
                                                        TOKHAAR_HOK, kiln);
                                        player.setRun(run);
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                @Override
                                public void run() {
                                    final WorldTile lookTo = getWorldTile(40,
                                            28);
                                    player.getPackets()
                                            .sendCameraLook(
                                                    Cutscene.getX(player,
                                                    ...
                                                    Cutscene.getY(player,
                                                            lookTo.getY()),
                                                    2200);
                                    final WorldTile posTile = getWorldTile(29,
                                            28);
                                    player.getPackets().sendCameraPos(
                                            Cutscene.getX(player,
                                                    posTile.getX()),
                                            Cutscene.getY(player,
                                                    posTile.getY()), 2500);
                                    final HarAken harAken = new HarAken(15211,
                                    ...
                                    GameTaskManager.scheduleTask(new GameTask(GameTask.ExecutionType.SCHEDULE, 5, TimeUnit.SECONDS) {
                                        ...
                                        public void run() {
                                            try {
                                                player.getDialogueManager().startDialogue(TokHaarHok.class, 6, TOKHAAR_HOK_SCENE, kiln);
                                            } catch (final Throwable e) {
                                                Logger.handle(e);
                                            }
                                        }
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                public void run() {
                                    ...
                                    player.getDialogueManager().startDialogue(
                                            TokHaarHok.class, 5, TOKHAAR_HOK_SCENE,
                                            kiln);
                                }
                                    ...
                                    if (count == 0) {
                                        ...
                                        player.getPackets().sendCameraPos(
                                                ...
                                                Cutscene.getY(player,
                                                        posTile.getY()), 3000);
                                        player.getDialogueManager()
                                                .startDialogue(TokHaarHok.class, 7,
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                @Override
                                public void run() {
                                    final WorldTile lookTo = getWorldTile(20,
                                            17);
                                    player.getPackets()
                                            .sendCameraLook(
                                                    Cutscene.getX(player,
                                                    ...
                                                    Cutscene.getY(player,
                                                            lookTo.getY()),
                                                    2500);
                                    final WorldTile posTile = getWorldTile(25,
                                            26);
                                    player.getPackets().sendCameraPos(
                                            Cutscene.getX(player,
                                                    posTile.getX()),
                                            Cutscene.getY(player,
                                                    posTile.getY()), 3000);
                                    player.getDialogueManager().startDialogue(
                                            TokHaarHok.class, 4, TOKHAAR_HOK_SCENE,
                                            kiln);
                                }
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                @Override
                                public void run() {
                                    final WorldTile lookTo = getWorldTile(31,
                                            43);
                                    player.getPackets()
                                            .sendCameraLook(
                                                    Cutscene.getX(player,
                                                    ...
                                                    Cutscene.getY(player,
                                                            lookTo.getY()),
                                                    2500);
                                    final WorldTile posTile = getWorldTile(31,
                                            34);
                                    player.getPackets().sendCameraPos(
                                            Cutscene.getX(player,
                                                    posTile.getX()),
                                            Cutscene.getY(player,
                                                    posTile.getY()), 4000);
                                    player.getDialogueManager().startDialogue(
                                            TokHaarHok.class, 3, TOKHAAR_HOK_SCENE,
                                            kiln);
                                }
                            ...
                            WorldTasksManager.schedule(new WorldTask() {
                                ...
                                @Override
                                public void run() {
                                    final WorldTile lookTo = getWorldTile(45,
                                            45);
                                    player.getPackets()
                                            .sendCameraLook(
                                                    Cutscene.getX(player,
                                                    ...
                                                    Cutscene.getY(player,
                                                            lookTo.getY()),
                                                    1000);
                                    final WorldTile posTile = getWorldTile(38,
                                            37);
                                    player.getPackets().sendCameraPos(
                                            Cutscene.getX(player,
                                                    posTile.getX()),
                                            Cutscene.getY(player,
                                                    posTile.getY()), 3000);
                                    player.getDialogueManager().startDialogue(
                                            TokHaarHok.class, 2, TOKHAAR_HOK_SCENE,
                                            kiln);
                                }
                                ...
                                public void run() {
                                    ...
                                    player.getPackets().sendCameraPos(
                                            ...
                                            Cutscene.getY(player,
                                                    posTile.getY()), 3000);
                                    player.getDialogueManager().startDialogue(
                                            TokHaarHok.class, 1, TOKHAAR_HOK_SCENE,
                                            kiln);
                                    stage = Stages.RUNNING;
                                    player.unlock();
                                }
    ...
    public WorldTile getTentacleTile() {
        final int corner = Utils.random(4);
    ...
    public WorldTile getSpawnTile(final int count, final int size) {
        final int position = count % 4;
        switch (position) {
            case 0: // east south
    ...
    public void checkCrystal() {
        if (stage != Stages.RUNNING)
            return;
        if (aliveNPCSCount == 1) {
            final int[] crystals = getLavaCrystal();
            if (crystals != null) {
                for (final int crystal : crystals) {
        ...
        FadingScreen.fade(player, new Runnable() {
            ...
            public void run() {
                ...
                if (getCurrentWave() == 38) {
                    final Integer reward = (Integer) player
                            .getTemporaryAttributtes().get("FightKilnReward");
                    if (reward != null) {
                        win();
                    }
                } else {
    ...
    public void nextWave() {
        if (stage != Stages.RUNNING)
            return;
        playMusic();
        final int nextWave = getCurrentWave() + 1;
        ...
        if (type == 0) {
            ...
            if (type == 1 || type == 4) {
                ...
                if (type == 4) {
                    ...
                    final Integer reward = (Integer) player
                            .getTemporaryAttributtes().get("FightKilnReward");
                    final int itemId = reward != null && reward == 1 ? 6571
                            : 23659;
    ...
    public boolean processNPCClick1(final NPC npc) {
        if (npc.getId() == 15195 && getCurrentWave() == 0) {
            player.getDialogueManager().startDialogue(TokHaarHok.class, 0,
                    TOKHAAR_HOK, this);
            return false;
        }
        return true;
