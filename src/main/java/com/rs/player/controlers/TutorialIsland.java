...
import com.rs.content.dialogues.impl.MasterChef;
...
public final class TutorialIsland extends Controller {
    ...
    private static final int RUNESCAPE_GUIDE_NPC = 945,
            SURVIVAL_EXPERT_NPC = 943, MASTER_CHEF_NPC = 942;
    ...
    public void refreshStage() {
        ...
        if (stage == 0 || stage == 1 || stage == 2) {
            final NPC guide = findNPC(RUNESCAPE_GUIDE_NPC);
            if (guide != null) {
                // than 1icon
            ...
            final NPC survival = findNPC(SURVIVAL_EXPERT_NPC);
            if (survival != null) {
                // than 1icon
            ...
            final NPC chef = findNPC(MASTER_CHEF_NPC);
            if (chef != null) {
    ...
    private NPC findNPC(final int id) {
    ...
    private void sendProgress() {
        ...
        if (stage == 0) {
            ...
            player.getPackets().sendIComponentText(372, 1, "");
            player.getPackets().sendIComponentText(372, 2,
                    "This is the base of the meals. To make dough we must mix");
            player.getPackets()
                    .sendIComponentText(372, 3,
    ...
    public void trackXP(final int skillId, final int addedXp) {
        if (getStage() == 9 && skillId == Skills.FIREMAKING) {
            updateProgress();
        } else if ((getStage() == 13 || getStage() == 16)
    ...
    public boolean canAddInventoryItem(final int itemId, final int amount) {
        if (getStage() == 7 && itemId == TreeDefinitions.NORMAL.getLogsId()) {
            updateProgress();
            player.getDialogueManager().startDialogue(ItemMessage.class,
    ...
    public boolean processObjectClick1(final WorldObject object) {
        if (object.getId() == 3014 && object.getX() == 3098
            ...
            player.lock(2);
            player.stopAll();
            player.addWalkSteps(player.getX() > object.getX() ? object.getX()
    ...
    public boolean processNPCClick1(final NPC npc) {
        if (npc.getId() == RUNESCAPE_GUIDE_NPC) {
            player.getDialogueManager().startDialogue(RuneScapeGuide.class,
                    RUNESCAPE_GUIDE_NPC, this);
            npc.faceEntity(player);
            npc.resetWalkSteps();
            return false;
        } else if (npc.getId() == SURVIVAL_EXPERT_NPC) {
            player.getDialogueManager().startDialogue(SurvivalExpert.class,
            ...
            player.getDialogueManager().startDialogue(MasterChef.class,
                    MASTER_CHEF_NPC, this);
            npc.faceEntity(player);
            npc.resetWalkSteps();
            return false;
        }
    ...
    public void sendInterfaces() {
        final int stage = getStage();
        final boolean rezi = player.getInterfaceManager().hasRezizableScreen();
