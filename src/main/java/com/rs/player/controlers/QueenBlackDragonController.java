...
public final class QueenBlackDragonController extends Controller {
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() == 70790) {
			...
			FadingScreen.fade(player, new Runnable() {
				...
				public void run() {
					...
					final boolean resizable = player.getInterfaceManager()
							.hasRezizableScreen();
		...
		if (object.getId() == 70815) {
			player.getDialogueManager().startDialogue(RewardChest.class, npc);
			return false;
		}
	...
	public void process() {
		...
		if (player.getY() < base.getY() + 28) {
			...
			if (platformStand++ == 3) {
				...
				player.applyHit(new Hit(npc, 200, Hit.HitLook.REGULAR_DAMAGE));
				platformStand = 0;
			}
		} else {
			platformStand = 0;
	...
	public boolean checkWalkStep(final int lastX, final int lastY,
		...
		if (npc != null && nextY < base.getY() + 28) {
			if (npc.getPhase() > 1) {
				for (final int[] step : PLATFORM_STEPS[0]) {
					if (base.getX() + (step[0] - 64) == nextX
							&& base.getY() + (step[1] - 64) == nextY)
						return true;
				}
				if (npc.getPhase() > 2) {
					for (final int[] step : PLATFORM_STEPS[1]) {
						if (base.getX() + (step[0] - 64) == nextX
								&& base.getY() + (step[1] - 64) == nextY)
							return true;
					}
					if (npc.getPhase() > 3) {
						for (final int[] step : PLATFORM_STEPS[2]) {
							if (base.getX() + (step[0] - 64) == nextX
									&& base.getY() + (step[1] - 64) == nextY)
								return true;
						}
		...
		case 1284:
			...
			case 10:
				for (int slot = 0; slot < npc.getRewards().toArray().length; slot++) {
					final Item item = npc.getRewards().get(slot);
					if (item == null) {
						continue;
					}
					boolean added = true;
					if (item.getDefinitions().isStackable()
						...
						for (int i = 0; i < item.getAmount(); i++) {
							final Item single = new Item(item.getId());
				...
				break;
			case 7:
				final Item item = npc.getRewards().get(slotId);
				if (item == null)
					return true;
				switch (packetId) {
				case 52:
	 ...
	 */
	private void end(final int type) {
