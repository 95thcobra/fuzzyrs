...
public class Foods {
    ...
    public static boolean eat(final Player player, final Item item,
                              final int slot) {
        ...
        if (!player.getControllerManager().canEat(food))
            return true;
        final String name = ItemDefinitions.getItemDefinitions(food.getId())
                .getName().toLowerCase();
        player.getPackets().sendGameMessage("You eat the " + name + ".");
        player.setNextAnimation(EAT_ANIM);
        final long foodDelay = name.contains("half") ? 600 : 1800;
    ...
    public enum Food {
        ...
        ROASTED_BIRD_MEAT(9980, 30, 0, null, 15),
         ...
         */
        Food(final int id, final int heal, final int newId) {
        ...
        Food(final int id, final int heal, final int newId,
             final Effect effect, final int extraHP) {
            this.id = id;
            this.heal = heal;
            this.newId = newId;
            this.effect = effect;
        ...
        Food(final int id, final int heal, final Effect effect) {
         ...
         */
        public static Food forId(final int itemId) {
    ...
    public enum Effect {
        ...
        GARDEN_PIE {
            ...
            public void effect(final Object object) {
                final Player player = (Player) object;
                final int level = player.getSkills().getLevel(Skills.FARMING);
                final int realLevel = player.getSkills().getLevelForXp(
                        Skills.FARMING);
        ...
        FISH_PIE {
            ...
            public void effect(final Object object) {
                final Player player = (Player) object;
                final int level = player.getSkills().getLevel(Skills.FISHING);
                final int realLevel = player.getSkills().getLevelForXp(
                        Skills.FISHING);
        ...
        WILD_PIE {
            ...
            public void effect(final Object object) {
                final Player player = (Player) object;
                final int level = player.getSkills().getLevel(Skills.SLAYER);
                final int realLevel = player.getSkills().getLevelForXp(
                        Skills.SLAYER);
                player.getSkills().set(Skills.SLAYER,
                        level >= realLevel ? realLevel + 4 : level + 4);
                final int level2 = player.getSkills().getLevel(Skills.RANGE);
                final int realLevel2 = player.getSkills().getLevelForXp(
                        Skills.RANGE);
        ...
        SPICY_STEW_EFFECT {
            ...
            public void effect(final Object object) {
                ...
                if (Utils.random(100) > 5) {
                    final int level = player.getSkills().getLevel(
                            Skills.COOKING);
                    final int realLevel = player.getSkills().getLevelForXp(
                            Skills.COOKING);
        ...
        POISION_KARMAMWANNJI_EFFECT {
            ...
            public void effect(final Object object) {
                final Player player = (Player) object;
                player.applyHit(new Hit(player, 50, Hit.HitLook.POISON_DAMAGE));
            }
        };
