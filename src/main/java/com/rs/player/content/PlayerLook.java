...
import com.rs.content.dialogues.impl.MakeOverMage;
...
public final class PlayerLook {
	...
	public static void openCharacterCustomizing(final Player player) {
		player.getPackets().sendWindowsPane(1028, 0); // character customizing
	}

	public static void handleCharacterCustomizingButtons(final Player player,
			final int buttonId) {
	...
	public static void handleMageMakeOverButtons(final Player player,
			final int buttonId) {
		if (buttonId == 14 || buttonId == 16 || buttonId == 15
			...
			final Boolean male = (Boolean) player.getTemporaryAttributtes()
					.remove("MageMakeOverGender");
			final Integer skin = (Integer) player.getTemporaryAttributtes()
					.remove("MageMakeOverSkin");
			player.closeInterfaces();
			if (male == null || skin == null)
				return;
			if (male == player.getAppearance().isMale()
					&& skin == player.getAppearance().getSkinColor()) {
				player.getDialogueManager().startDialogue(MakeOverMage.class, 2676,
						1);
			} else {
				player.getDialogueManager().startDialogue(MakeOverMage.class, 2676,
						2);
	...
	public static void handleHairdresserSalonButtons(final Player player,
			final int buttonId, final int slotId) {// Hair and color match
													// button count so just loop
													// and
													// do ++, but cant find
													// button ids
		if (buttonId == 6) {
			...
			final Boolean hairSalon = (Boolean) player
					.getTemporaryAttributtes().get("hairSaloon");
			if (hairSalon != null && hairSalon) {
				player.getAppearance().setHairStyle(
						(int) ClientScriptMap.getMap(
								player.getAppearance().isMale() ? 2339 : 2342)
								.getKeyForValue(slotId / 2));
			} else if (player.getAppearance().isMale()) {
				player.getAppearance().setBeardStyle(
						ClientScriptMap.getMap(703).getIntValue(slotId / 2));
			}
		} else if (buttonId == 16) {
			player.getAppearance().setHairColor(
					ClientScriptMap.getMap(2345).getIntValue(slotId / 2));
		}
	}

	public static void openMageMakeOver(final Player player) {
	...
	public static void handleThessaliasMakeOverButtons(final Player player,
			final int buttonId, final int slotId) {
		if (buttonId == 6) {
			player.getTemporaryAttributtes().put("ThessaliasMakeOver", 0);
		} else if (buttonId == 7) {
			if (ClientScriptMap.getMap(
					player.getAppearance().isMale() ? 690 : 1591)
			...
			if (player.getControllerManager().getController() instanceof StartTutorial
					&& (int) player.getControllerManager().getController()
							.getArguments()[0] == 4) {
				player.getControllerManager().getController().getArguments()[0] = 5;
			}
			player.closeInterfaces();
		} else if (buttonId == 12) { // set part
			final Integer stage = (Integer) player.getTemporaryAttributtes()
					.get("ThessaliasMakeOver");
			if (stage == null || stage == 0) {
				...
				if (!player.getAppearance().isMale()) {
					player.getAppearance().setBeardStyle(
							player.getAppearance().getTopStyle());
				}
				player.getAppearance().setArmsStyle(
						player.getAppearance().isMale() ? 26 : 65); // default
				player.getAppearance().setWristsStyle(
						player.getAppearance().isMale() ? 34 : 68); // default
			} else if (stage == 1) {
				player.getAppearance().setArmsStyle(
						ClientScriptMap.getMap(
								player.getAppearance().isMale() ? 711 : 693)
								.getIntValue(slotId / 2));
			} else if (stage == 2) {
				player.getAppearance().setWristsStyle(
				...
				player.getAppearance().setLegsStyle(
						ClientScriptMap.getMap(
								player.getAppearance().isMale() ? 1586 : 1607)
								.getIntValue(slotId / 2));
			}
			...
			if (stage == null || stage == 0 || stage == 1) {
				player.getAppearance().setTopColor(
						ClientScriptMap.getMap(3282).getIntValue(slotId / 2));
			} else if (stage == 3) {
				player.getAppearance().setLegsColor(
