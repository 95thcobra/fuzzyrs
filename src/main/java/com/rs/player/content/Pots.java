...
public final class Pots {
    ...
    public static Pot getPot(final int id) {
        for (final Pot pot : Pot.values()) {
            for (final int potionId : pot.id) {
                if (id == potionId)
                    return pot;
            }
        }
    ...
    public static boolean pot(final Player player, final Item item,
                              final int slot) {
        ...
        final int toPot = pot.id.length - dosesLeft;
        ...
        for (final int skillId : pot.effect.affectedSkills) {
    ...
    public static boolean mixPotion(final Player player, final int usedSlot,
                                    final int withSlot) {
        /*
         * Item itemUsed = player.getInventory().getItem(usedSlot); Item
		 * usedWith = player.getInventory().getItem(withSlot); Pot first =
		 * getPot(itemUsed.getMessageIcon()); Pot second = getPot(usedWith.getMessageIcon());
		 ...
		 * false; } else if (itemUsed.getMessageIcon() == 229)// empty vial return false;
		 */
        return true;
    }

    public static void resetOverLoadEffect(final Player player) {
        if (!player.isDead()) {
    ...
    public static void applyOverLoadEffect(final Player player) {
    ...
    public enum Pot {
        ...
        Pot(final int[] id, final Effects effect) {
            this.id = id;
            this.effect = effect;
        }
    }
    ...
    private enum Effects {
        ATTACK_POTION(Skills.ATTACK) {
            @Override
            public int getAffectedSkill(final Player player, final int skillId,
                                        final int actualLevel, final int realLevel) {
        ...
        SUMMONING_POT(Skills.SUMMONING) {
            ...
            public int getAffectedSkill(final Player player, final int skillId,
                                        final int actualLevel, final int realLevel) {
                final int restore = (int) (Math.floor(player.getSkills()
        ...
        ENERGY_POTION() {
            @Override
            public void extra(final Player player) {
                final int restoredEnergy = player.getRunEnergy() + 20;
        ...
        SUPER_ENERGY() {
            @Override
            public void extra(final Player player) {
                final int restoredEnergy = player.getRunEnergy() + 40;
                player.setRunEnergy(restoredEnergy > 100 ? 100 : restoredEnergy);
            }
        },
        ANTI_FIRE() {
        ...
        EXTREME_STR_POTION(Skills.STRENGTH) {
            @Override
            public boolean canDrink(final Player player) {
                if (player.getControllerManager().getController() instanceof Wilderness
                        || player.getControllerManager().getController() instanceof CrucibleController
                        || FfaZone.isOverloadChanged(player)) {
                    player.getPackets().sendGameMessage(
        ...
        RECOVER_SPECIAL() {
            ...
            public boolean canDrink(final Player player) {
                ...
                if (!player.getRank().getDonateRank().isNone()) {
                    player.getPackets().sendGameMessage(
                            "You need to be a donator to drink this potion.");
                    return false;
                }
                final Long time = (Long) player.getTemporaryAttributtes().get(
                        "Recover_Special_Pot");
        ...
        SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK,
            ...
            public int getAffectedSkill(final Player player, final int skillId,
                ...
                if (skillId == Skills.DEFENCE) {
                    final int boost = (int) (realLevel * 0.25);
                    final int level = actualLevel > realLevel ? realLevel
                            : actualLevel;
                    return level + boost;
                } else
            ...
            @Override
            public void extra(final Player player) {
                final int hitpointsModification = (int) (player
                        .getMaxHitpoints() * 0.15);
        ...
        OVERLOAD() {
            ...
            public void extra(final Player player) {
                ...
                player.getPackets().sendHideIComponent(PotionTimerInterface.POTION_TIMER_INTERFACE_ID, PotionType.OVERLOAD.getImageId(), false);
                WorldTasksManager.schedule(new WorldTask() {
                    int count = 4;
        ...
        PRAYER_RENEWAL() {
            ...
            public void extra(final Player player) {
                ...
                player.getPackets().sendHideIComponent(PotionTimerInterface.POTION_TIMER_INTERFACE_ID, PotionType.PRAYER_RENEWAL.getImageId(), false);
            }
        },
        ...
        Effects(final int... affectedSkills) {
            this(null, affectedSkills);
        }

        Effects(final String drinkMessage, final int... affectedSkills) {
