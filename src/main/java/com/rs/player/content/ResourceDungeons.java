			...
			if (o.getId() == d.getObjectId()) {
				if (player.getSkills().getLevel(Skills.DUNGEONEERING) >= d.getLvlReq()) {
					...
					if (!player.getSeenDungeon()[d.getNumber()]) {
						...
						player.sendMessage("You've gained "+d.getXp()+" xp in Dungeoneering for uncovering the secrets of "
						+ "a new dungeon!");
						return true;
					}
					return true;
				} else {
					player.sendMessage("You need a Dungeoneering level of "+d.getLvlReq()+" to uncover the secrets of this "
					+ "dungeon.");
					return true;
				}
			}
