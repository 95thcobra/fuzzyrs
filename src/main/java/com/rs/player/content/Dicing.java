...
public class Dicing {
	...
	public static void privateRoll(final Player player, final int itemId,
			final int graphic, final int lowest, final int highest) {
	...
	public static int getRandom(final int lowest, final int highest) {
		final Random r = new Random();
		if (lowest > highest)
			return -1;
		final long range = (long) highest - (long) lowest + 1;
		final long fraction = (long) (range * r.nextDouble());
		return (int) (fraction + lowest);
	}

	public static String diceText(final int id) {
		switch (id) {
		case 15086:
			return "a six-sided";
		case 15088:
	...
	public static int getGraphic(final int id) {
		return 0;
	}
}
