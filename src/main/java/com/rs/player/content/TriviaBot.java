...
public class TriviaBot {
	...
	private static String questions[][] = {
			...
			{ "What is the Matrix gametask playing mode? (Multiplayer/Solo)",
					"Multiplayer" },
	...
	public static void Run() {
		final int rand = RandomQuestion();
		questionid = rand;
		victory = false;
		for (final Player participant : World.getPlayers()) {
			if (participant == null) {
				continue;
			}
	...
	public static void sendRoundWinner(final String winner) {
		for (final Player participant : World.getPlayers()) {
			if (participant == null) {
				continue;
			}
	...
	public static void verifyAnswer(final Player player, final String answer) {
		if (victory) {
			player.getPackets()
					.sendGameMessage(
	...
	public static boolean TriviaArea(final Player participant) {
		return participant.getX() >= 2630 && participant.getX() <= 2660
