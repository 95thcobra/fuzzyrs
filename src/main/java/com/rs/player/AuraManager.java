...
public class AuraManager implements Serializable {
     ...
     * return seconds
     */
    public static int getActivationTime(final int aura) {
        switch (aura) {
            case 20958:
    ...
    public static int getCooldown(final int aura) {
        switch (aura) {
            case 23854:
                return 1;
            case 23874:
    ...
    public static int getTier(final int aura) {
        switch (aura) {
            case 23874:
            case 23876:
            case 23854:
	...
	/*
     * public long getCoolDown(int aura) { Long coolDown = cooldowns.get(aura);
	 * if (coolDown == null) return 0; return coolDown; }
	 */
    ...
    public long getCoolDown(final int aura) {
        return 0;
    }

    public void activate() {
        final Item item = player.getEquipment().getItem(Equipment.SLOT_AURA);
        if (item == null)
            return;
        ...
        final int toId = getTransformIntoAura(item.getId());
        if (toId != -1) {
            ...
            if (Utils.currentTimeMillis() <= getCoolDown(item.getId()))
                ...
                return;
            final int tier = getTier(item.getId());
    ...
    public int getTransformIntoAura(final int aura) {
        switch (aura) {
            case 23896: // infernal
                return 23880;
            case 23880: // infernal
    ...
    public void sendAuraRemainingTime() {
        if (!isActivated()) {
            final long cooldown = getCoolDown(player.getEquipment().getAuraId());
    ...
    public String getFormatedTime(long seconds) {
        ...
        final String minutesString = (minutes < 10 ? "0" : "") + minutes;
        final String secondsString = (seconds < 10 ? "0" : "") + seconds;
        return hours + ":" + minutesString + ":" + secondsString;
    }

    public void sendTimeRemaining(final int aura) {
        final long cooldown = getCoolDown(aura);
    ...
    public int getAuraModelId2() {
        final int aura = player.getEquipment().getAuraId();
        switch (aura) {
    ...
    public int getActiveGraphic(final int tier) {
        if (tier == 2)
            return 1764;
        if (tier == 3)
            return 1763;
    ...
    public void checkSuccefulHits(final int damage) {
        if (!isActivated() || World.isPvpArea(player))
            return;
        final int aura = player.getEquipment().getAuraId();
        if (aura == 22296) {
    ...
    public void useVampyrism(final int damage) {
        final int heal = (int) (damage * 0.05);
        if (heal > 0) {
            player.heal(heal);
        }
    }
     ...
     * @return {@code True}.
     */
    public boolean isWingedAura(final int aura) {
        switch (aura) {
            case 22905: // Corruption.
            case 22899: // Salvation.
            case 23848: // Harmony.
