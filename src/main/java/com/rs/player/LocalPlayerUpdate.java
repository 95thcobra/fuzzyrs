...
public final class LocalPlayerUpdate {
	...
	public LocalPlayerUpdate(final Player player) {
		this.player = player;
	...
	public boolean needAppearenceUpdate(final int index, final byte[] hash) {
	...
	public void init(final OutputStream stream) {
	...
	private boolean needsRemove(final Player p) {
		return (p.hasFinished() || !player.withinDistance(p,
				player.hasLargeSceneView() ? 126 : 14));
	}

	private boolean needsAdd(final Player p) {
		return p != null
				&& !p.hasFinished()
	...
	private void updateRegionHash(final OutputStream stream,
			final int lastRegionHash, final int currentRegionHash) {
		final int lastRegionX = lastRegionHash >> 8;
		final int lastRegionY = 0xff & lastRegionHash;
		final int lastPlane = lastRegionHash >> 16;
		final int currentRegionX = currentRegionHash >> 8;
		final int currentRegionY = 0xff & currentRegionHash;
		...
		if (lastRegionX == currentRegionX && lastRegionY == currentRegionY) {
			...
			int opcode;
			final int dx = currentRegionX - lastRegionX;
			final int dy = currentRegionY - lastRegionY;
			if (dx == -1 && dy == -1) {
				opcode = 0;
			} else if (dx == 1 && dy == -1) {
				opcode = 2;
			...
			final int xOffset = currentRegionX - lastRegionX;
			final int yOffset = currentRegionY - lastRegionY;
	...
	private void processOutsidePlayers(final OutputStream stream,
			final OutputStream updateBlockData, final boolean nsn2) {
		stream.initBitAccess();
		int skip = 0;
		localAddedPlayers = 0;
		for (int i = 0; i < outPlayersIndexesCount; i++) {
			final int playerIndex = outPlayersIndexes[i];
			...
			if (needsAdd(p)) {
				...
				final int hash = p.getRegionHash();
				...
				final boolean needAppearenceUpdate = needAppearenceUpdate(
				...
				final int hash = p == null ? regionHashes[playerIndex] : p
						.getRegionHash();
				if (p != null && hash != regionHashes[playerIndex]) {
					...
					for (int i2 = i + 1; i2 < outPlayersIndexesCount; i2++) {
						final int p2Index = outPlayersIndexes[i2];
						if (nsn2 ? (0x1 & slotFlags[p2Index]) == 0
								: (0x1 & slotFlags[p2Index]) != 0) {
							continue;
						}
						final Player p2 = World.getPlayers().get(p2Index);
						if (needsAdd(p2)
	...
	private void processLocalPlayers(final OutputStream stream,
			final OutputStream updateBlockData, final boolean nsn0) {
		stream.initBitAccess();
		int skip = 0;
		for (int i = 0; i < localPlayersIndexesCount; i++) {
			final int playerIndex = localPlayersIndexes[i];
			...
			if (needsRemove(p)) {
				...
				final boolean needAppearenceUpdate = needAppearenceUpdate(
						p.getIndex(), p.getAppearance()
						.getMD5AppeareanceDataHash());
				final boolean needUpdate = p.needMasksUpdate()
				...
				if (p.hasTeleported()) {
					...
					final int planeOffset = p.getPlane()
					...
					for (int i2 = i + 1; i2 < localPlayersIndexesCount; i2++) {
						final int p2Index = localPlayersIndexes[i2];
						if (nsn0 ? (0x1 & slotFlags[p2Index]) != 0
								: (0x1 & slotFlags[p2Index]) == 0) {
							continue;
						}
						final Player p2 = localPlayers[p2Index];
						if (needsRemove(p2)
								|| p2.hasTeleported()
	...
	private void skipPlayers(final OutputStream stream, final int amount) {
	...
	private void appendUpdateBlock(final Player p, final OutputStream data,
			final boolean needAppearenceUpdate, final boolean added) {
		int maskData = 0;
	...
	private void applyForceTalkMask(final Player p, final OutputStream data) {
		data.writeString(p.getNextForceTalk().getText());
	}

	private void applyHitsMask(final Player p, final OutputStream data) {
		final int count = p.getNextHits().size();
		data.writeByteC(count);
		if (count > 0) {
			int hp = p.getHitpoints();
			final int maxHp = p.getMaxHitpoints();
			if (hp > maxHp) {
				hp = maxHp;
			}
			final int hpBarPercentage = maxHp == 0 ? 0 : (hp * 255 / maxHp);
			for (final Hit hit : p.getNextHits()) {
				if (hit.getDamage() < 0) {
					hit.setDamage(0);
				}
				final boolean interactingWith = hit.interactingWith(player, p);
	...
	private void applyFaceEntityMask(final Player p, final OutputStream data) {
		data.writeShort128(p.getNextFaceEntity() == -2 ? p.getLastFaceEntity()
				: p.getNextFaceEntity());
	}

	private void applyFaceDirectionMask(final Player p, final OutputStream data) {
		data.writeShort(p.getDirection()); // also works as face tile as dir
		// calced on setnextfacetile
	}

	private void applyMoveTypeMask(final Player p, final OutputStream data) {
		data.write128Byte(p.getRun() ? 2 : 1);
	}

	private void applyTemporaryMoveTypeMask(final Player p,
			final OutputStream data) {
		data.writeByte128(p.getTemporaryMoveType());
	}

	private void applyGraphicsMask1(final Player p, final OutputStream data) {
	...
	private void applyGraphicsMask2(final Player p, final OutputStream data) {
	...
	private void applyGraphicsMask3(final Player p, final OutputStream data) {
	...
	private void applyGraphicsMask4(final Player p, final OutputStream data) {
	...
	private void applyAnimationMask(final Player p, final OutputStream data) {
		for (final int id : p.getNextAnimation().getIds()) {
			data.writeBigSmart(id);
		}
		data.writeByte(p.getNextAnimation().getSpeed());
	}

	private void applyAppearanceMask(final Player p, final OutputStream data) {
		final byte[] renderData = p.getAppearance().getAppeareanceData();
	...
	private void applyForceMovementMask(final Player p, final OutputStream data) {
	...
	public OutputStream createPacketAndProcess() {
		...
		final Player player = localPlayers[playerIndex];
		if (player == null) {
			outPlayersIndexes[outPlayersIndexesCount++] = playerIndex;
		} else {
