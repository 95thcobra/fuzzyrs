...
public final class WorldThread extends Thread {
	...
	public String[] news = {"Reminder -- only donate to <img=1>Gircat"
			,"Speak to the Wizard located at home for teleporting"
			,"Use your Loyalty tokens on titles, aura's, and other items"
			,"Real world trading and vote selling are not allowed"
			,"Use the friends chat to communicate and ask for help"};
		...
		while (!CoresManager.shutdown) {
			...
			try {
				...
				if (NewsTimer == 0){
					if (newsnum  == 5)
						newsnum = 0;
					World.sendWorldWideMessage("<col=800000><img=3>[Server] "+news[newsnum]+".");
					NewsTimer = 360;
					newsnum++;
				}
				if (NewsTimer > 0){
