        ...
        if (nex.isFollowTarget()) {
                    ...
                    public void run() {
                        if (count == 0) {
                            ...
                            nex.setNextForceMovement(new ForceMovement(dir, 1,
                                    ...
                                    : idx == 1 ? 3 : 2));
                            for (final Entity entity : nex
                                    .calculatePossibleTargets(center, dir,
                                            idx == 0 || idx == 2)) {
            ...
            if (Utils.getRandom(15) == 0) {
                ...
                for (final Entity t : npc.getPossibleTargets()) {
                    if (t instanceof Player) {
                        final int thisDistance = Utils.getDistance(t.getX(),
            ...
            if (nex.getAttacksStage() == 0) {
                ...
                if (!nex.isTrapsSettedUp() && Utils.getRandom(2) == 0) {
                    ...
                    final ArrayList<Entity> possibleTargets = nex
                            .getPossibleTargets();
                ...
                npc.setNextAnimation(new Animation(17413));
                for (final Entity t : npc.getPossibleTargets()) {
                    final int distance = Utils.getDistance(t.getX(), t.getY(),
                            npc.getX(), npc.getY());
                    if (distance <= 10) {
                        final int damage = 800 - (distance * 800 / 11);
                            ...
                            if (x == player.getX() && y == player.getY()) {
                                ...
                                for (final Entity t : npc.getPossibleTargets()) {
                                    World.sendProjectile(npc, t, 374, 41, 16,
                                            41, 35, 16, 0);
                                    final int damage = CombatScript.getRandomMaxHit(npc,
                ...
                if (nex.getLastSiphon() < Utils.currentTimeMillis()
                    ...
                    final int bloodReaverSize = NPCDefinitions
                            .getNPCDefinitions(13458).size;
                    int respawnedBloodReaverCount = 0;
                    final int maxMinions = Utils.getRandom(3);
                    if (maxMinions != 0) {
                        final int[][] dirs = Utils
