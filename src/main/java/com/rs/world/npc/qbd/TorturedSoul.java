...
public final class TorturedSoul extends NPC {
     ...
     */
    public TorturedSoul(final QueenBlackDragon dragon, final Player victim,
                        final WorldTile spawn) {
     ...
     */
    public void specialAttack(final WorldTile teleport) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                setNextForceTalk(FORCE_MESSAGES[Utils
                ...
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    @Override
                    public void run() {
                        final WorldTile current = new WorldTile(x, y, 1);
                        victim.getPackets().sendGraphics(SPECIAL_GRAPHIC,
                                current);
                        Entity target = null;
                        for (final TorturedSoul soul : dragon.getSouls()) {
                        ...
                        if (target == null) {
                            for (final NPC worm : dragon.getWorms()) {
                        ...
                        if (target != null) {
                            stop();
                            target.applyHit(new Hit(dragon, Utils.random(200,
                                    260), HitLook.REGULAR_DAMAGE));
                            return;
                        }
                        if (x > victim.getX()) {
                            x--;
