...
public final class TimeStopAttack implements QueenAttack {
    ...
    public int attack(final QueenBlackDragon npc, final Player victim) {
        ...
        if (npc.getSouls().isEmpty())
            return 1;
        final TorturedSoul soul = npc.getSouls().get(
                Utils.random(npc.getSouls().size()));
        soul.setNextWorldTile(Utils.random(2) == 0 ? npc.getBase().transform(
                24, 28, 0) : npc.getBase().transform(42, 28, 0));
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (stage == 8) {
                    stop();
                    npc.getTemporaryAttributtes().put("_time_stop_atk",
                            npc.getTicks() + Utils.random(50) + 40);
                    for (final TorturedSoul s : npc.getSouls()) {
                        s.setLocked(false);
                    }
                    for (final NPC worm : npc.getWorms()) {
