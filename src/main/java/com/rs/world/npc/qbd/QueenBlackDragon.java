...
public final class QueenBlackDragon extends NPC {
     ...
     * @param tile     The worldtask tile to set the queen on.
     * @param base     The dynamic region's base location.
     */
    public QueenBlackDragon(final Player attacker, final WorldTile tile,
                            final WorldTile base) {
    ...
    @Override
    public void finish() {
        for (final TorturedSoul s : souls) {
            s.finish();
        }
        for (final NPC worm : worms) {
            worm.finish();
        }
        super.finish();
    }
            ...
            public void run() {
                ...
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    public void run() {
                        if (getPhase() > 4)
                            return;
                        final NPC worm = new NPC(15464, destination, -1, true,
                                true);
                        worms.add(worm);
     ...
     */
    public void setState(final QueenState state) {
        this.state = state;
    }
     ...
     */
    public void switchState(final QueenState state) {
        this.state = state;
        if (state.getMessage() != null) {
            final String[] messages = state.getMessage().split("(nl)");
     ...
     */
    public void openRewardChest(final boolean replace) {
     ...
     */
    public void setNextAttack(final int nextAttack) {
    ...
    public void setPhase(final int phase) {
        ...
        switch (phase) {
            ...
            case 5:
                setCantInteract(true);
                for (final TorturedSoul soul : souls) {
                    soul.finish();
                }
                for (final NPC worm : worms) {
                    worm.finish();
    ...
    private void prepareRewards() {
        ...
        final List<Item> rewardTable = new ArrayList<Item>();
        for (final int[] reward : REWARDS) {
            final Item item = new Item(reward[0], reward[1]
     ...
     */
    public void setSpawningWorms(final boolean spawningWorms) {
        if (!spawningWorms) {
     ...
     */
    public void setActiveArtifact(final WorldObject activeArtifact) {
