...
public final class Pet extends NPC {
     ...
     */
    public Pet(final int id, final int itemId, final Player owner,
               final WorldTile tile, final PetDetails details) {
    ...
    public void growNextStage() {
        ...
        if (pet == null)
            return;
        final int npcId = pet.getNpcId(details.getStage() + 1);
        if (npcId < 1)
            return;
        details.setStage(details.getStage() + 1);
        final int itemId = pet.getItemId(details.getStage());
        ...
        finish();
        final Pet newPet = new Pet(npcId, itemId, owner, owner, details);
     ...
     */
    /*
	 * public void pickup() { owner.getInventory().addItem(itemId, 1);
	 * owner.setPet(null); owner.getPetManager().setNpcId(-1);
	 * owner.getPetManager().setItemId(-1); switchOrb(false);
	 * owner.getPackets().closeInterface(
	 * owner.getInterfaceManager().hasRezizableScreen() ? 98 : 212);
	 * owner.getPackets().sendIComponentSettings(747, 17, 0, 0, 0); finish(); }
	 */
    public void pickup() {
        ...
        owner.getPackets().closeInterface(
                owner.getInterfaceManager().hasRezizableScreen() ? 98 : 212);
        owner.getPackets().sendIComponentSettings(747, 17, 0, 0, 0);
        owner.getInterfaceManager().sendSquealOfFortune();
        finish();
    }
     ...
     */
    public void switchOrb(final boolean enable) {
