...
public final class ItemsContainer<T extends Item> implements Serializable {
    ...
    public ItemsContainer(final int size, final boolean alwaysStackable) {
        data = new Item[size];
        this.alwaysStackable = alwaysStackable;
    }

    public void shift() {
        final Item[] oldData = data;
    ...
    public void set(final int slot, final T item) {
        if (slot < 0 || slot >= data.length)
            return;
        data[slot] = item;
    }

    public void set2(final int slot, final Item item) {
        if (slot < 0 || slot >= data.length)
            return;
        data[slot] = item;
    }

    public boolean forceAdd(final T item) {
        for (int i = 0; i < data.length; i++) {
            if (data[i] == null) {
                data[i] = item;
                return true;
    ...
    public boolean containsOne(final T item) {
        for (final Item aData : data) {
            if (aData != null) {
    ...
    public Item[] getItemsCopy() {
        final Item[] newData = new Item[data.length];
        System.arraycopy(data, 0, newData, 0, newData.length);
        return newData;
    }

    public ItemsContainer<Item> asItemContainer() {
        final ItemsContainer<Item> c = new ItemsContainer<Item>(data.length,
    ...
    public int getThisItemSlot(final T item) {
        for (int i = 0; i < data.length; i++) {
            if (data[i] != null) {
                if (data[i].getId() == item.getId())
                    return i;
        ...
        if (data[preferredSlot] != null) {
            if (data[preferredSlot].getId() == item.getId()) {
                ...
                if (amt > toRemove) {
                    ...
                    // Item(data[preferredSlot].getDefinition().getMessageIcon(), amt);
            ...
            if (data[i] != null) {
                if (data[i].getId() == item.getId()) {
                    ...
                    if (amt > toRemove) {
                        ...
                        amt -= toRemove;
                        toRemove = 0;
                        // data[i] = new Item(data[i].getDefinition().getMessageIcon(),
                        // amt);
    ...
    public void addAll(final ItemsContainer<T> container) {
        for (int i = 0; i < container.getSize(); i++) {
            final T item = container.get(i);
            if (item != null) {
                this.add(item);
            }
        }
    ...
    public boolean hasSpaceFor(final ItemsContainer<T> container) {
        for (int i = 0; i < container.getSize(); i++) {
            final T item = container.get(i);
            if (item != null) {
    ...
    private boolean hasSpaceForItem(final T item) {
        if (alwaysStackable || item.getDefinitions().isStackable()
                || item.getDefinitions().isNoted()) {
            for (final Item aData : data) {
                if (aData != null) {
    ...
    public boolean goesOverAmount(final ItemsContainer<T> container) {
        for (int i = 0; i < container.getSize(); i++) {
